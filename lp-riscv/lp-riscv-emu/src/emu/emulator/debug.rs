    ...
    pub fn format_debug_info(&self, highlight_pc: Option<u32>, log_count: usize) -> String {
        ...
        if let Some(error_pc) = highlight_pc {
            ...
            for i in (0..code.len()).step_by(4) {
                if i + 4 <= code.len() {
                    ...
                    let disasm = lp_riscv_inst::format_instruction(inst_word);
        ...
        if !self.log_buffer.is_empty() {
            ...
            let max_inst_width = logs_to_show
                .iter()
                .map(|log| {
                    let disassembly = lp_riscv_inst::format_instruction(log.instruction());
                    disassembly.len()
                })
                .max()
                .unwrap_or(0);
            ...
            for log in logs_to_show {
                ...
                let disassembly = lp_riscv_inst::format_instruction(instruction);
