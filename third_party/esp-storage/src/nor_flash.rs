...
use crate::{
    buffer::uninit_slice_mut,
    FlashStorage, FlashStorageError,
};
#[cfg(not(feature = "panic-unaligned-buffer"))]
use crate::buffer::{FlashSectorBuffer, uninit_slice};
...
impl FlashStorage<'_> {
    ...
    fn read_word_aligned(&mut self, offset: u32, bytes: &mut [u8]) -> Result<(), FlashStorageError> {
    ...
    fn read_unaligned(&mut self, offset: u32, bytes: &mut [u8]) -> Result<(), FlashStorageError> {
        #[cfg(feature = "panic-unaligned-buffer")]
        {
            let _ = (offset, bytes);
            panic!("esp-storage read called with unaligned output buffer");
        }
    ...
    fn write_word_aligned(&mut self, offset: u32, bytes: &[u8]) -> Result<(), FlashStorageError> {
        for (offset, chunk) in (offset..)
            .step_by(Self::SECTOR_SIZE as _)
    ...
    #[cold]
    #[inline(never)]
    fn write_unaligned(&mut self, offset: u32, bytes: &[u8]) -> Result<(), FlashStorageError> {
        #[cfg(feature = "panic-unaligned-buffer")]
        {
            let _ = (offset, bytes);
            panic!("esp-storage write called with unaligned input buffer");
        }
...
impl ReadNorFlash for FlashStorage<'_> {
    ...
    fn read(&mut self, offset: u32, bytes: &mut [u8]) -> Result<(), Self::Error> {
        ...
        if Self::is_word_aligned(bytes) {
            self.read_word_aligned(offset, bytes)?;
        } else {
            self.read_unaligned(offset, bytes)?;
        }
...
impl NorFlash for FlashStorage<'_> {
    ...
    fn write(&mut self, offset: u32, bytes: &[u8]) -> Result<(), Self::Error> {
        ...
        if Self::is_word_aligned(bytes) {
            self.write_word_aligned(offset, bytes)?;
        } else {
