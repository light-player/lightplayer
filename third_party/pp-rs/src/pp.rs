    ...
    fn start_define_invocation(
        ...
        if let Some(define) = lexer.get_define(name) {
            let mut invocation = DefineInvocation {
                ...
                parameter_expanding: core::usize::MAX,
            };
        ...
        if let Some(invocation) = self.define_invocations.last_mut() {
            ...
            if let Some(argument) = invocation.parameters.get(invocation.parameter_expanding) {
                if let Some(token) = argument.get(invocation.parameter_position) {
                    ...
                    invocation.parameter_expanding = core::usize::MAX;
...
impl<'a> Preprocessor<'a> {
    ...
    pub fn add_define(
        ...
        self.macro_expansion_enabled = true;
        self.directive_processor.add_define(name, content)
    }

    fn should_expand_macros(&mut self) -> bool {
        if !self.directive_processor.defines.is_empty() {
            self.macro_expansion_enabled = true;
        }
        self.macro_expansion_enabled
    }

    fn step_without_macros(&mut self) -> Step<Token> {
        let token = self.directive_processor.step()?;
            ...
            if name == "__LINE__" {
                return Ok(Token {
                    value: TokenValue::Integer(Integer {
                        value: self
                            .directive_processor
                            .apply_line_offset(token.location.line, token.location)?
                            as u64,
                        signed: false,
                        width: 32,
                    }),
    ...
    fn step(&mut self) -> Step<Token> {
        if !self.should_expand_macros() {
            return self.step_without_macros();
        }
    ...
    #[cfg(test)]
    pub(crate) fn is_macro_expansion_enabled(&self) -> bool {
        self.macro_expansion_enabled
    }
}
