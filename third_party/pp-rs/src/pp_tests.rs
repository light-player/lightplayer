...
#[test]
fn macro_expansion_stays_disabled_without_defines() {
    let mut pp = Preprocessor::new("#version 450 core\n#line 10\nfoo __LINE__");

    let tokens: Vec<PreprocessorItem> = pp.by_ref().collect();

    assert!(!pp.is_macro_expansion_enabled());
    assert!(matches!(
        tokens.first(),
        Some(Ok(Token {
            value: TokenValue::Version(_),
            ..
        }))
    ));
    assert!(matches!(
    ...
    assert!(matches!(
        tokens.get(2),
        Some(Ok(Token {
            value: TokenValue::Integer(Integer { value: 11, .. }),
            ..
        }))
    ));
}
...
#[test]
fn source_define_enables_macro_expansion() {
    let mut pp = Preprocessor::new("#define A 7\nA");

    assert!(matches!(
        pp.next(),
        Some(Ok(Token {
            value: TokenValue::Integer(Integer { value: 7, .. }),
            ..
        }))
    ));
    assert!(pp.is_macro_expansion_enabled());
