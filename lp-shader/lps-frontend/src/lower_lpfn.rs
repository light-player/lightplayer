...
fn walk_block_for_lpfn_calls(
    ...
    for stmt in block.iter() {
        match stmt {
            ...
            Statement::Block(inner) => walk_block_for_lpfn_calls(module, func, inner, seen),
            Statement::If { accept, reject, .. } => {
                walk_block_for_lpfn_calls(module, func, accept, seen);
                walk_block_for_lpfn_calls(module, func, reject, seen);
            }
            Statement::Switch { cases, .. } => {
                for c in cases {
                    walk_block_for_lpfn_calls(module, func, &c.body, seen);
                }
            }
            Statement::Loop {
                body, continuing, ..
            } => {
                walk_block_for_lpfn_calls(module, func, body, seen);
                walk_block_for_lpfn_calls(module, func, continuing, seen);
            }
            _ => {}
        }
    }
...
fn lpfn_glsl_params_csv(module: &Module, callee: Handle<Function>) -> Result<String, LowerError> {
    let f = &module.functions[callee];
    let mut out = Vec::new();
    for arg in &f.arguments {
        out.push(lpfn_glsl_param_token(module, arg.ty)?);
    }
    Ok(out.join(","))
}

fn lpfn_glsl_param_token(module: &Module, ty: Handle<naga::Type>) -> Result<String, LowerError> {
    match &module.types[ty].inner {
        TypeInner::Pointer { base, .. } => lpfn_pointee_token(module, *base),
        ...
        TypeInner::Vector { size, scalar, .. } => lpfn_vector_token(*size, scalar.kind),
...
fn lpfn_pointee_token(module: &Module, base_ty: Handle<naga::Type>) -> Result<String, LowerError> {
...
fn lpfn_vector_token(size: VectorSize, kind: ScalarKind) -> Result<String, LowerError> {
    Ok(match (size, kind) {
...
fn build_lpfn_import_decl(
    ...
    let lpfn_glsl_params = Some(lpfn_glsl_params_csv(module, callee)?);
...
fn lpfn_arg_kinds(
    module: &Module,
    callee: Handle<Function>,
) -> Result<Vec<LpfnArgKind>, LowerError> {
    let f = &module.functions[callee];
    let mut out = Vec::new();
    for arg in &f.arguments {
        match &module.types[arg.ty].inner {
            ...
            TypeInner::Vector { size, scalar, .. } => {
                let _ = naga_scalar_to_ir_type(scalar.kind)?;
                out.push(LpfnArgKind::ValueVector(vector_size_usize(*size) as u8));
            }
            _ => {
                return Err(LowerError::UnsupportedType(String::from(
...
pub(crate) fn lower_lpfn_call(
    ...
    let callee_ref = *ctx.lpfn_map.get(&callee).ok_or_else(|| {
        LowerError::Internal(format!(
            "LPFX callee {:?} not registered",
            ctx.module.functions[callee].name
        ))
    })?;
    let kinds = lpfn_arg_kinds(ctx.module, callee)?;
