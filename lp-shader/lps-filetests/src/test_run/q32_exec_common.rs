...
pub(crate) fn glsl_q32_to_glsl_value(ty: &LpsType, q: &LpsValueF64) -> Result<LpsValue, GlslError> {
    ...
    Ok(match (ty, q) {
        ...
        (Array { element, len }, LpsValueF64::Array(items)) => {
            if items.len() != *len as usize {
                return Err(bad());
            }
...
pub(crate) fn call_f32_from_q32<E: Q32ShaderExecutable>(
    ...
    match ret.value {
        Some(LpsValueF64::Float(x)) => Ok(x as f32),
...
pub(crate) fn call_i32_from_q32<E: Q32ShaderExecutable>(
    ...
    let ret = exec.call_q32_ret(name, args)?;
    match ret.value {
        Some(LpsValueF64::Int(x)) => Ok(x),
        Some(LpsValueF64::UInt(x)) => Ok(x as i32),
        other => Err(GlslError::new(
            ErrorCode::E0400,
...
pub(crate) fn call_vec_from_q32<E: Q32ShaderExecutable>(
    ...
    let ret = exec.call_q32_ret(name, args)?;
    match ret.value {
        Some(LpsValueF64::Vec2(a, b)) if dim == 2 => Ok(vec![a as f32, b as f32]),
        Some(LpsValueF64::Vec3(a, b, c)) if dim == 3 => Ok(vec![a as f32, b as f32, c as f32]),
        Some(LpsValueF64::Vec4(a, b, c, d)) if dim == 4 => {
...
pub(crate) fn call_ivec_from_q32<E: Q32ShaderExecutable>(
    ...
    let ret = exec.call_q32_ret(name, args)?;
    match (&ret.value, dim) {
        (Some(LpsValueF64::IVec2(a, b)), 2) => Ok(vec![*a, *b]),
        (Some(LpsValueF64::IVec3(a, b, c)), 3) => Ok(vec![*a, *b, *c]),
        (Some(LpsValueF64::IVec4(a, b, c, d)), 4) => Ok(vec![*a, *b, *c, *d]),
        _ => Err(GlslError::new(
            ErrorCode::E0400,
...
pub(crate) fn call_uvec_from_q32<E: Q32ShaderExecutable>(
    ...
    let ret = exec.call_q32_ret(name, args)?;
    match (&ret.value, dim) {
        (Some(LpsValueF64::UVec2(a, b)), 2) => Ok(vec![*a, *b]),
        (Some(LpsValueF64::UVec3(a, b, c)), 3) => Ok(vec![*a, *b, *c]),
        (Some(LpsValueF64::UVec4(a, b, c, d)), 4) => Ok(vec![*a, *b, *c, *d]),
        _ => Err(GlslError::new(
            ErrorCode::E0400,
...
pub(crate) fn call_bvec_from_q32<E: Q32ShaderExecutable>(
    ...
    let ret = exec.call_q32_ret(name, args)?;
    match (&ret.value, dim) {
        (Some(LpsValueF64::BVec2(a, b)), 2) => Ok(vec![*a, *b]),
        (Some(LpsValueF64::BVec3(a, b, c)), 3) => Ok(vec![*a, *b, *c]),
        (Some(LpsValueF64::BVec4(a, b, c, d)), 4) => Ok(vec![*a, *b, *c, *d]),
        _ => Err(GlslError::new(
            ErrorCode::E0400,
...
pub(crate) fn call_mat_from_q32<E: Q32ShaderExecutable>(
    ...
    let flat: Vec<f32> = match &ret.value {
        Some(LpsValueF64::Mat2(a)) if n == 4 => a.iter().map(|x| *x as f32).collect(),
        Some(LpsValueF64::Mat3(a)) if n == 9 => a.iter().map(|x| *x as f32).collect(),
        Some(LpsValueF64::Mat4(a)) if n == 16 => a.iter().map(|x| *x as f32).collect(),
        other => {
            return Err(GlslError::new(
                ErrorCode::E0400,
...
pub(crate) fn call_array_from_q32<E: Q32ShaderExecutable>(
    ...
    let ret = exec.call_q32_ret(name, args)?;
    match ret.value {
        Some(LpsValueF64::Array(items)) if items.len() == len => items
            .iter()
