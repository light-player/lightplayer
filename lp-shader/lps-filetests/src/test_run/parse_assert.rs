...
fn array_element_matches_base(base: &str, v: &LpsValueF32) -> bool {
    match base {
        "float" => matches!(v, LpsValueF32::F32(_)),
        "int" => matches!(v, LpsValueF32::I32(_)),
        "uint" => matches!(v, LpsValueF32::U32(_)),
        "bool" => matches!(v, LpsValueF32::Bool(_)),
        "vec2" => matches!(v, LpsValueF32::Vec2(_)),
        "vec3" => matches!(v, LpsValueF32::Vec3(_)),
        "vec4" => matches!(v, LpsValueF32::Vec4(_)),
        "ivec2" => matches!(v, LpsValueF32::IVec2(_)),
        "ivec3" => matches!(v, LpsValueF32::IVec3(_)),
        "ivec4" => matches!(v, LpsValueF32::IVec4(_)),
        "uvec2" => matches!(v, LpsValueF32::UVec2(_)),
        "uvec3" => matches!(v, LpsValueF32::UVec3(_)),
        "uvec4" => matches!(v, LpsValueF32::UVec4(_)),
        "bvec2" => matches!(v, LpsValueF32::BVec2(_)),
        "bvec3" => matches!(v, LpsValueF32::BVec3(_)),
        "bvec4" => matches!(v, LpsValueF32::BVec4(_)),
        "mat2" => matches!(v, LpsValueF32::Mat2x2(_)),
        "mat3" => matches!(v, LpsValueF32::Mat3x3(_)),
        "mat4" => matches!(v, LpsValueF32::Mat4x4(_)),
        _ => false,
    }
}
...
fn parse_typed_array_constructor(s: &str) -> Result<Option<LpsValueF32>> {
    let s = s.trim();
...
pub fn parse_glsl_value(s: &str) -> Result<LpsValueF32> {
    ...
    if s.ends_with('u') || s.ends_with('U') {
        ...
        if let Ok(u) = num_str.parse::<u32>() {
            return Ok(LpsValueF32::U32(u));
        }
    }

    // Try parsing as integer
    if let Ok(i) = s.parse::<i32>() {
        return Ok(LpsValueF32::I32(i));
    }
    ...
    match s {
        "true" => return Ok(LpsValueF32::Bool(true)),
        "false" => return Ok(LpsValueF32::Bool(false)),
        _ => {}
    }
...
pub fn parse_function_arguments(arg_strings: &[String]) -> Result<Vec<LpsValueF32>> {
    arg_strings
        .iter()
...
fn format_glsl_value(value: &LpsValueF32) -> String {
...
mod tests {
    ...
    fn test_parse_glsl_value_int() {
        assert!(parse_glsl_value("42").unwrap().eq(&LpsValueF32::I32(42)));
        assert!(parse_glsl_value("-10").unwrap().eq(&LpsValueF32::I32(-10)));
    }

    #[test]
    fn test_parse_glsl_value_float() {
        let v1 = parse_glsl_value("3.14").unwrap();
        assert!(v1.approx_eq(&LpsValueF32::F32(3.14), 0.001));
        let v2 = parse_glsl_value("-0.5").unwrap();
        assert!(v2.approx_eq(&LpsValueF32::F32(-0.5), 0.001));
    }

    #[test]
    fn test_parse_glsl_value_uint() {
        assert!(parse_glsl_value("42u").unwrap().eq(&LpsValueF32::U32(42)));
        assert!(parse_glsl_value("100U").unwrap().eq(&LpsValueF32::U32(100)));
    }

    #[test]
    fn test_parse_glsl_value_bool() {
        assert!(
            parse_glsl_value("true")
                .unwrap()
                .eq(&LpsValueF32::Bool(true))
        );
        assert!(
            parse_glsl_value("false")
                .unwrap()
                .eq(&LpsValueF32::Bool(false))
        );
    }
    ...
    fn test_compare_results_exact_match() {
        let actual = LpsValueF32::I32(42);
        let expected = LpsValueF32::I32(42);
    ...
    fn test_compare_results_approx_mismatch() {
        let actual = LpsValueF32::F32(1.0);
        let expected = LpsValueF32::F32(1.1);
