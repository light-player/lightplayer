...
// ============================================================================
// lpfn_rgb2hsv(): Convert RGB color space to HSV
// ============================================================================

float test_lpfn_rgb2hsv_pure_red() {
    // RGB(1, 0, 0) -> HSV(0, 1, 1) approximately
    vec3 rgb = vec3(1.0, 0.0, 0.0);
    vec3 hsv = lpfn_rgb2hsv(rgb);
...
// run: test_lpfn_rgb2hsv_pure_red() == 1.0

float test_lpfn_rgb2hsv_black() {
    // RGB(0, 0, 0) -> HSV(0, 0, 0)
...
// run: test_lpfn_rgb2hsv_black() == 1.0

float test_lpfn_rgb2hsv_white() {
    // RGB(1, 1, 1) -> HSV(0, 0, 1)
...
// run: test_lpfn_rgb2hsv_white() == 1.0

float test_lpfn_rgb2hsv_grayscale() {
    // Grayscale colors should have saturation = 0
    vec3 gray1 = vec3(0.5, 0.5, 0.5);
    vec3 gray2 = vec3(0.3, 0.3, 0.3);
    vec3 hsv1 = lpfn_rgb2hsv(gray1);
    vec3 hsv2 = lpfn_rgb2hsv(gray2);
    bool valid = hsv1.y < 0.01 && hsv2.y < 0.01;
    return valid ? 1.0 : 0.0;
}

// run: test_lpfn_rgb2hsv_grayscale() == 1.0

float test_lpfn_rgb2hsv_vec4() {
    // Test vec4 version preserves alpha
    vec4 rgb = vec4(1.0, 0.0, 0.0, 0.7);
    vec4 hsv = lpfn_rgb2hsv(rgb);
    bool valid = abs(hsv.a - 0.7) < 0.01;
    return valid ? 1.0 : 0.0;
}

// run: test_lpfn_rgb2hsv_vec4() == 1.0

float test_lpfn_rgb2hsv_range() {
    // HSV components should be in [0, 1] range
    vec3 rgb1 = vec3(1.0, 0.0, 0.0);
    vec3 rgb2 = vec3(0.5, 0.3, 0.8);
    vec3 hsv1 = lpfn_rgb2hsv(rgb1);
    vec3 hsv2 = lpfn_rgb2hsv(rgb2);
...
// run: test_lpfn_rgb2hsv_range() == 1.0
