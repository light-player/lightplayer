...
// ============================================================================
// lpfn_srandom(): Signed random noise functions
// ============================================================================

float test_lpfn_srandom_1d() {
    // Test 1D signed random - should be in [-1, 1] range
    float x = 0.5;
    uint seed = 0u;
    float n = lpfn_srandom(x, seed);
...
float test_lpfn_srandom_3d_vec() {
    ...
    vec3 p = vec3(0.5, 0.5, 0.5);
    uint seed = 0u;
    vec3 n = lpfn_srandom3_vec(p, seed);
...
float test_lpfn_srandom_3d_tile() {
    ...
    uint seed = 0u;
    vec3 n = lpfn_srandom3_tile(p, tileLength, seed);
    bool valid = n.x >= -1.0 && n.x <= 1.0 &&
                 n.y >= -1.0 && n.y <= 1.0 &&
                 n.z >= -1.0 && n.z <= 1.0;
    return valid ? 1.0 : 0.0;
...
float test_lpfn_srandom_deterministic() {
    ...
    float n1 = lpfn_srandom(0.5, 0u);
    float n2 = lpfn_srandom(0.5, 0u);
...
float test_lpfn_srandom_different_seeds() {
    ...
    float diff1 = abs(lpfn_srandom(0.5, 0u) - lpfn_srandom(0.5, 1u));
    float diff2 = abs(lpfn_srandom(1.5, 0u) - lpfn_srandom(1.5, 1u));
