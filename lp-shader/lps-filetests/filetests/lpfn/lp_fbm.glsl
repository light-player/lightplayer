...
// ============================================================================
// lpfn_fbm(): Fractional Brownian Motion noise functions
// ============================================================================

float test_lpfn_fbm_2d() {
    // Test 2D FBM - should be in reasonable range
    vec2 p = vec2(0.5, 0.5);
    int octaves = 4;
    uint seed = 0u;
    float n = lpfn_fbm(p, octaves, seed);
...
float test_lpfn_fbm_3d_tile() {
    ...
    int octaves = 4;
    uint seed = 0u;
    float n = lpfn_fbm(p, tileLength, octaves, seed);
    return (n >= -5.0 && n <= 5.0) ? 1.0 : 0.0;
}
...
float test_lpfn_fbm_deterministic() {
    ...
    float n1 = lpfn_fbm(vec2(0.5, 0.5), 4, 0u);
    float n2 = lpfn_fbm(vec2(0.5, 0.5), 4, 0u);
...
float test_lpfn_fbm_different_seeds() {
    ...
    float diff1 = abs(lpfn_fbm(vec2(0.5, 0.5), 4, 0u) - lpfn_fbm(vec2(0.5, 0.5), 4, 1u));
    float diff2 = abs(lpfn_fbm(vec2(1.5, 1.5), 4, 0u) - lpfn_fbm(vec2(1.5, 1.5), 4, 1u));
...
float test_lpfn_fbm_octaves_effect() {
    ...
    float n1 = lpfn_fbm(vec2(0.5, 0.5), 1, 0u);
    float n2 = lpfn_fbm(vec2(0.5, 0.5), 4, 0u);
...
float test_lpfn_fbm_smoothness() {
    ...
    float n1 = lpfn_fbm(vec2(0.5, 0.5), 4, 0u);
    float n2 = lpfn_fbm(vec2(0.51, 0.5), 4, 0u);
    float diff = abs(n1 - n2);
