...
use lps_filetests::util::{format_glsl_value, parse_lps_value_literal};
...
fn test_format_mat2x2() {
    ...
    match (mat, parsed) {
        (LpsValueF32::Mat2x2(m1), LpsValueF32::Mat2x2(m2)) => {
            assert_eq!(m1, m2);
        }
...
fn test_format_mat3x3() {
    ...
    let parsed = parse_lps_value_literal(&formatted).unwrap();
    match (mat, parsed) {
        (LpsValueF32::Mat3x3(m1), LpsValueF32::Mat3x3(m2)) => {
            assert_eq!(m1, m2);
        }
        _ => panic!("Parsed value should match original"),
    }
...
fn test_format_mat4x4() {
    ...
    let parsed = parse_lps_value_literal(&formatted).unwrap();
    match (mat, parsed) {
        (LpsValueF32::Mat4x4(m1), LpsValueF32::Mat4x4(m2)) => {
            assert_eq!(m1, m2);
        }
        _ => panic!("Parsed value should match original"),
    }
