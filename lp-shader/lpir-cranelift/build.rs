//! When `riscv32-emu` is enabled, embeds `lp-glsl-builtins-emu-app` for linking tests.
...
fn main() {
    ...
    if !exe_path.exists() {
        println!(
            "cargo:warning=lp-glsl-builtins-emu-app not found at {} — run scripts/build-builtins.sh",
            exe_path.display()
        );
        std::fs::write(&out_path, "pub const LP_BUILTINS_EXE_BYTES: &[u8] = &[];\n")
    ...
    let copied = std::path::Path::new(&out_dir).join("lp-glsl-builtins-emu-app");
