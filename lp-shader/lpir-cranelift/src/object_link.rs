...
/// Embedded `lp-glsl-builtins-emu-app` bytes (empty if missing at build time).
...
pub fn link_object_with_builtins(object_elf: &[u8]) -> Result<ElfLoadInfo, CompilerError> {
    ...
    if builtins_exe_bytes.is_empty() {
        return Err(CompilerError::Codegen(CompileError::unsupported(
            "lp-glsl-builtins-emu-app is empty or was not found at build time; run scripts/build-builtins.sh from the workspace root",
        )));
    }
