...
| `[lpvm-wasm](lpvm-wasm/)`                         | LPIR → WASM (`wasm-encoder`) for browser / `wasm.q32` filetests.            |
| `[lps-q32](lps-q32/)`                             | Q16.16 fixed-point types: `Q32`, vectors, matrices, helpers (`no_std`).     |
...
| `[lpvm](lpvm/)`                                   | Runtime traits (`LpvmEngine`, `LpvmModule`, `LpvmInstance`).                 |
...
**Test-only / host helpers:** `lpvm`, `lps-shared`, `lps-diagnostics`,
`lps-filetests`*, `lpvm-wasm` (as used by filetests and web demo).
