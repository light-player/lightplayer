...
fn find_filetests_dir() -> Result<PathBuf> {
    // Try relative to this crate's manifest (lps-filetests is sibling)
    if let Ok(manifest_dir) = std::env::var("CARGO_MANIFEST_DIR") {
        let manifest_path = PathBuf::from(&manifest_dir);
        // From lp-shader/lps-filetests-gen-app, go to lp-shader/lps-filetests/filetests
        let filetests = manifest_path
            .parent()
            .and_then(|p| p.parent())
            .map(|lps| lps.join("lps-filetests").join("filetests"));
    ...
    let candidates = vec![
        ...
        PathBuf::from("../lp-shader/lps-filetests/filetests"),
        PathBuf::from("lps-filetests/filetests"),
        PathBuf::from("../lps-filetests/filetests"),
        PathBuf::from("lightplayer/crates/lps-filetests/filetests"),
        PathBuf::from("crates/lps-filetests/filetests"),
    ];
    ...
    loop {
        let candidate = search_dir.join("../../lps-filetests/filetests");
        if !candidate.exists() {
            let alt = search_dir.join("lightplayer/crates/lps-filetests/filetests");
