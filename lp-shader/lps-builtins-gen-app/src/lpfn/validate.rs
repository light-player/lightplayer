...
pub fn validate_lpfn_functions(
    parsed_functions: &[ParsedLpfnFunction],
) -> Result<(), LpfnCodegenError> {
    // Check for missing attributes (should have been caught earlier, but double-check)
    for func in parsed_functions {
        if !func.info.has_lpfn_impl_attr {
            return Err(LpfnCodegenError::MissingAttribute {
...
fn validate_decimal_pairs(parsed_functions: &[ParsedLpfnFunction]) -> Result<(), LpfnCodegenError> {
    // Group functions by full signature (name + types), not just name
    // This allows overloaded functions (e.g., vec3 and vec4 variants)
    let mut by_signature: HashMap<String, Vec<&ParsedLpfnFunction>> = HashMap::new();
    ...
    for functions in by_signature.values() {
        ...
        if has_variant {
            ...
            if !has_f32 {
                ...
                return Err(LpfnCodegenError::MissingPair {
...
fn validate_signature_consistency(
    ...
    for functions in by_signature.values() {
        ...
        let mut f32_func: Option<&ParsedLpfnFunction> = None;
        let mut q32_func: Option<&ParsedLpfnFunction> = None;
                ...
                match variant {
                    Variant::F32 => {
                        if let Some(f32_func_ref) = f32_func {
                            return Err(LpfnCodegenError::DuplicateFunctionName {
        ...
        {
            return Err(LpfnCodegenError::SignatureMismatch {
...
fn validate_overload_distinct_signatures(
    ...
    let mut by_name: HashMap<String, Vec<&ParsedLpfnFunction>> = HashMap::new();
    ...
    for (name, functions) in &by_name {
        ...
        let mut by_sig_and_variant: HashMap<
            (String, Option<crate::lpfn::errors::Variant>),
            Vec<&ParsedLpfnFunction>,
        > = HashMap::new();
