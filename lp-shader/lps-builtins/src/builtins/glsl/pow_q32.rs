...
/// - pow(x, -y) = 1 / pow(x, y)
#[unsafe(no_mangle)]
pub extern "C" fn __lps_pow_q32(x: i32, y: i32) -> i32 {
    ...
    let log2_x = __lps_log2_q32(x);
    let product = __lp_lpir_fmul_q32(log2_x, y);
    __lps_exp2_q32(product)
}
...
mod tests {
    ...
    fn test_pow_helper(inputs: &[(f32, f32, f32)], tolerance: f32, min_tolerance: f32) {
        for (x, y, expected) in inputs {
            ...
            let result_fixed = __lps_pow_q32(x_fixed, y_fixed);
