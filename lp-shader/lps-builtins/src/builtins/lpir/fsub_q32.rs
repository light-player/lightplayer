...
//! wrapping `sub` when the shader opts into
...
#[unsafe(no_mangle)]
pub extern "C" fn __lp_lpir_fsub_q32(a: i32, b: i32) -> i32 {
...
mod tests {
    ...
    fn test_basic_subtraction() {
        ...
        for (a, b, expected) in tests {
            ...
            let result_fixed = __lp_lpir_fsub_q32(a_fixed, b_fixed);
    ...
    fn test_zero_handling() {
        ...
        assert_eq!(__lp_lpir_fsub_q32(one, zero), one, "1 - 0 should be 1");
        assert_eq!(
            __lp_lpir_fsub_q32(zero, one),
            float_to_fixed(-1.0),
            "0 - 1 should be -1"
        );
        assert_eq!(__lp_lpir_fsub_q32(zero, zero), 0, "0 - 0 should be 0");
    }
