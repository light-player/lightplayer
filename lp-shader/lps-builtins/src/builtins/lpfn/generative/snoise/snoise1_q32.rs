...
//!
//! ```glsl
//! float noise = lpfn_snoise(5.0, 123u);
//! ```
//!
//! # Parameters
//!
...
//!
//! The user-facing `lpfn_snoise` function maps to internal `__lp_lpfn_snoise1` which
...
pub fn lpfn_snoise(x: Q32, seed: u32) -> Q32 {
    ...
    let hash = lpfn_hash(cell as u32, seed);
...
#[lpfn_impl_macro::lpfn_impl(q32, "float lpfn_snoise(float x, uint seed)")]
#[unsafe(no_mangle)]
pub extern "C" fn __lp_lpfn_snoise1_q32(x: i32, seed: u32) -> i32 {
    lpfn_snoise(Q32::from_fixed(x), seed).to_fixed()
}
...
mod tests {
    ...
    fn test_simplex1_basic() {
        ...
        let results: Vec<i32> = (0..10)
            .map(|i| __lp_lpfn_snoise1_q32(float_to_fixed(i as f32 * 0.5), 0))
            .collect();
        ...
        let result_seed0 = __lp_lpfn_snoise1_q32(float_to_fixed(5.0), 0);
        let result_seed1 = __lp_lpfn_snoise1_q32(float_to_fixed(5.0), 1);
    ...
    fn test_simplex1_different_seeds() {
        ...
        let hash0 = lpfn_hash(0, 0);
        let hash1 = lpfn_hash(0, 1);
        ...
        for i in 0..100 {
            ...
            let result_seed0 = __lp_lpfn_snoise1_q32(x, 0);
            let result_seed1 = __lp_lpfn_snoise1_q32(x, 1);
    ...
    fn test_simplex1_output_grid() {
        ...
        for row in 0..10 {
            let x = row as f32;
            let result = __lp_lpfn_snoise1_q32(float_to_fixed(x), 0);
        ...
        for row in 0..10 {
            let x = row as f32;
            let result = __lp_lpfn_snoise1_q32(float_to_fixed(x), 1);
            let result_float = fixed_to_float(result);
            println!("  x={:4.1}: {:7.4}", x, result_float);
        }
        ...
        for seed in 0..5 {
            let result = __lp_lpfn_snoise1_q32(x, seed);
            let result_float = fixed_to_float(result);
        ...
        for i in 0..100 {
            let x = float_to_fixed(i as f32 * 0.1);
            let result = __lp_lpfn_snoise1_q32(x, 0);
            let result_float = fixed_to_float(result);
            assert!(
    ...
    fn test_simplex1_deterministic() {
        let result1 = __lp_lpfn_snoise1_q32(float_to_fixed(42.5), 123);
        let result2 = __lp_lpfn_snoise1_q32(float_to_fixed(42.5), 123);
