...
//! 1996 SIGGRAPH paper. Our implementation uses our own lpfn_hash2 function.
//!
//! # GLSL Usage
//!
...
//!
//! ```glsl
//! float noise = lpfn_worley(vec2(5.0, 3.0), 123u);
//! ```
//!
//! # Parameters
//!
...
//!
//! The Worley algorithm checks neighboring cells (9 cells in 2D) to find the nearest
...
//!
//! Therefore:
//! - `dx`, `dy` are bounded by approximately [-2, 2]
//! - `dx*dx`, `dy*dy` are bounded by approximately 4.0
//! - `dist_sq = dx*dx + dy*dy` is bounded by approximately 8.0
...
pub fn lpfn_worley2(p: Vec2Q32, seed: u32) -> Q32 {
    let x = p.x;
    let y = p.y;
    ...
    if range_x < distance {
        ...
        let test_index = lpfn_hash2(test_x_int as u32, test_y_int as u32, seed);
        let test_point = get_point_2d(test_index as usize, test_x_int, test_y_int);
        // Range analysis: test_dx, test_dy bounded by ~[-2, 2] (same as primary distance)
...
#[lpfn_impl_macro::lpfn_impl(q32, "float lpfn_worley(vec2 p, uint seed)")]
#[unsafe(no_mangle)]
pub extern "C" fn __lp_lpfn_worley2_q32(x: i32, y: i32, seed: u32) -> i32 {
    let p = Vec2Q32::new(Q32::from_fixed(x), Q32::from_fixed(y));
    lpfn_worley2(p, seed).to_fixed()
}
...
mod tests {
    ...
    fn test_worley2_basic() {
        let result1 = __lp_lpfn_worley2_q32(float_to_fixed(1.5), float_to_fixed(2.3), 0);
        let result2 = __lp_lpfn_worley2_q32(float_to_fixed(3.7), float_to_fixed(2.3), 0);
        let result3 = __lp_lpfn_worley2_q32(float_to_fixed(1.5), float_to_fixed(2.3), 1);
    ...
    fn test_worley2_range() {
        ...
        for i in 0..50 {
            ...
            let result = __lp_lpfn_worley2_q32(x, y, 0);
    ...
    fn test_worley2_deterministic() {
        let result1 = __lp_lpfn_worley2_q32(float_to_fixed(42.5), float_to_fixed(37.3), 123);
        let result2 = __lp_lpfn_worley2_q32(float_to_fixed(42.5), float_to_fixed(37.3), 123);
