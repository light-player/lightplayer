...
Migrate `lp-engine` from `lps-cranelift` to `lpvm-cranelift` and validate
...
- **lp-engine migration:**
    - Replace `lps-cranelift` dependency with `lpvm-cranelift`
...
- **lp-server Cargo.toml:**
    - Forward new features (`std`, `cranelift-optimizer`, `cranelift-verifier`)
      from `lpvm-cranelift` instead of `lps-cranelift`
- **fw-emu validation:**
...
- `no_std` / embedded readiness of `lpvm-cranelift` itself (Stage VI-A —
  prerequisite)
...
- `lp-engine` compiles against `lpvm-cranelift` instead of `lps-cranelift`
...
- Stage VI-A (lpvm-cranelift embedded readiness) — `std` feature gating,
