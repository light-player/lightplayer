# Stage V1: RV32 Object, Linking, and Emulator (in `lpir-cranelift`)

## Goal

Add RISC-V object emission, builtins ELF linking, and emulator execution **inside
`lpir-cranelift`**, validated by **in-crate tests**. This runs **before** filetest
integration (Stage V2) so switching the filetest runner to `lpir-cranelift` does
not strand the `rv32.q32` / emulator path on the old compiler.

## Suggested plan name

`lpir-cranelift-stage-v1`

## Scope

**In scope:**
- **Object emission** in `lpir-cranelift`:
...
- Emulator smoke tests **inside `lpir-cranelift`**
