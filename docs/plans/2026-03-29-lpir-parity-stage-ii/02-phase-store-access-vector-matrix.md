...
Handle **`Statement::Store`** when the **pointer** is **`Expression::Access`** (and any *
*`Load(Access…)`** wrapper Naga uses) for:
...
1. **`lower_stmt.rs` — `Statement::Store`**
    - Add arms (order narrow → broad):
        ...
        - If Naga uses **`Store(Load(Access…))`** or similar, peel **`Load`** the same way as *
          *`lower_expr`**.
