...
Confirm the exact **Naga** expression trees for failing cases (matrix `++`, `bvec[i]`,
`Load(Access…)`), then implement **read** lowering for **`Expression::Access`** on **local** vectors
and matrices (and extend **`Load`** peeling as needed).

## Code organization reminders

- Prefer a small **`lower_access`** section or module: **one concept per function** (vector scalar
  pick vs matrix column vs matrix element).
