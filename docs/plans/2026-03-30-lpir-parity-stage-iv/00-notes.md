...
Target: Make `array/phase/1-foundation.glsl`, `2-bounds-checking.glsl`, and `3-initialization.glsl` pass.
...
**Context:** Phase 1-3 tests don't include function parameters. Fat pointers are only needed when arrays are passed to functions. The Phase 7 tests cover array function parameters.
...
**Suggested:** B) Defer fat pointers. Phase 1-3 focuses on local arrays only. This keeps the plan focused and allows validating the core slot-based approach first.

### Q2: Bounds check implementation - branch or select?

**Context:** Need to implement clamping: `clamped = if index < 0 then 0 else if index >= len then len-1 else index`
...
**Suggested:** B) Select chain. Arrays are already using slots (slower than VRegs), and select chains are easier to generate and optimize later. Branching requires label management.
...
**Suggested:** A) Individual stores for v1. Simpler implementation, and for typical LED shader arrays (small), the difference is negligible. Can optimize with `Memcpy` later if needed.
...
- `lp-glsl/lp-glsl-naga/src/lower_ctx.rs` - Add array_map, slot allocation
- `lp-glsl/lp-glsl-naga/src/lower_expr.rs` - Add array Access/AccessIndex
- `lp-glsl/lp-glsl-naga/src/lower_stmt.rs` - Add array Store
- `lp-glsl/lp-glsl-naga/src/lower_array.rs` - NEW: Array lowering helpers
- `lp-glsl/lp-glsl-naga/src/naga_util.rs` - Add array type utilities
