...
We have:
- `lp-builtins` crate with q32 implementations for `div`, `mul`, `sqrt`
...
Refactor GLSL builtin math functions to call `lp-builtins` functions directly instead of using GLSL intrinsics. We need both float and q32 implementations, with float being simple wrappers around Rust std math functions.
