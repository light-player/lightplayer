...
Refactor the codebase to clean up serial code and move shared constants into `lp-emu-shared`:

1. **Move SYSCALL constants** from `lp-emu-guest` to `lp-emu-shared`
    - Currently defined in `lp-riscv/lp-emu-guest/src/syscall.rs`
...
- **Location**: `lp-riscv/lp-emu-guest/src/syscall.rs`
- **Constants defined**:
    - `SYSCALL_PANIC = 1`
    - `SYSCALL_WRITE = 2`
    - `SYSCALL_DEBUG = 3`
...
1. **What goes in lp-emu-shared**: ✅ **RESOLVED**
...
2. **Error codes**: ✅ **RESOLVED** - Define error code constants in
   `lp-emu-shared/src/guest_serial.rs`
