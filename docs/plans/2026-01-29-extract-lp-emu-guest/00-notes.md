...
The guest code currently lives in `lp-glsl/apps/lp-builtins-app/src/`:
...
- `lp-builtins` crate - Provides builtin functions that need to be referenced
...
- `lp-builtins-app` currently uses this code as its main binary
- `lp-glsl-compiler` builds `lp-builtins-app` and embeds it as a library
...
- `lp-builtins-app` needs to use this code as its main binary
...
- `builtin_refs.rs` is auto-generated by `lp-builtin-gen`
...
- `lp-builtins-app` is currently built and embedded by `lp-glsl-compiler`
