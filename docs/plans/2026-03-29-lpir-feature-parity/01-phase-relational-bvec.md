...
- `builtins/common-isnan.glsl` and `builtins/common-isinf.glsl` (component-wise relational on vectors)

## Code organization reminders

- Prefer granular helpers (e.g. one small function per `RelationalFunction` group) in `lower_expr.rs` or a dedicated submodule if the file grows.
...
3. **Tests**
   - Run: `./scripts/glsl-filetests.sh vec/bvec2/fn-all.glsl` (and siblings), `vec/bvec2/op-equal.glsl`,
...
```bash
./scripts/glsl-filetests.sh vec/bvec3 vec/bvec4
```
