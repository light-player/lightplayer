...
- `GlslType` (or equivalent) in `lpir` / `lp-glsl-naga` for `mat2`, `mat3`, `mat4`
...
2. **`lp-glsl-naga/src/lib.rs`** (and any `extract_functions` helpers) — extend
...
Targeted filetests once a matrix file compiles past the type error (may still fail until phases 3–4):
...
(if `lp-glsl-naga` / `lpir` public API changes affect the firmware graph)
