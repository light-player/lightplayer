...
- `GlslType` (or equivalent) in `lpir` / `lps-naga` for `mat2`, `mat3`, `mat4`
...
2. **`lps-naga/src/lib.rs`** (and any `extract_functions` helpers) — extend
...
(if `lps-naga` / `lpir` public API changes affect the firmware graph)
