...
- **Runtime**: `LpsTexture2DDescriptor` is the role-neutral `repr(C)` guest descriptor `{ ptr, width, height, row_stride }` built from `LpsTextureBuf`.
...
- **`Texture2D` vs descriptor**: `Texture2D` is the logical shader/metadata type. The ABI value is the four-`u32` `LpsTexture2DDescriptor`; metadata and diagnostics stay logical (no public “fake struct” of `ptr`/`width` fields in the type surface the way a user-defined struct would).
