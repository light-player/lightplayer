...
We implemented LP library functions (`lpfn_snoise1`, `lpfn_snoise2`, `lpfn_snoise3`) but missed a
...
- **Currently we only support q32** - so we have `__lp_q32_lpfn_snoise3` implementations
...
- Option A: `__lp_float_lpfn_snoise3` (explicit float prefix)
- Option B: `__lpfn_snoise3` (no float prefix, just the base name)
- Option C: `lpfn_snoise3` (user-facing name, no prefix)
- Option D: Something else?

**Suggested Answer**: Option B (`__lpfn_snoise3`) - matches the pattern used for `sin`/`cos` where
`"sinf"` maps to `"__lp_sin"` in the mapping table. The float nature is implicit since we're in
float codegen before the transform.

**ANSWERED**: Option B (`__lpfn_snoise3`) - confirmed by user. This matches
...
- Option A: Use `LpLibFn::symbol_name()` to get `"__lpfn_snoise3"`, emit TestCase call similar to
...
`"__lp_q32_lpfn_snoise3"` (the actual function name). The mapping table maps `"__lpfn_snoise3"` (
...
1. Map `"__lpfn_snoise3"` → `BuiltinId::LpSimplex3` via `map_testcase_to_builtin()`
2. Use `BuiltinId::LpSimplex3.name()` to get `"__lp_q32_lpfn_snoise3"`
3. Look up `"__lp_q32_lpfn_snoise3"` in `func_id_map` to get the FuncId
...
- Generate registry entries based on actual function names (which may be `__lp_q32_lpfn_snoise3`)

### Q4: Hash Function Handling

**Context**: Hash functions (`lpfn_hash`) are not float-specific - they operate on integers. They're
...
2. What the q32 implementation name is (e.g., `__lp_q32_lpfn_snoise3` for simplex functions)
...
- `q32_name(&self) -> Option<&'static str>` - returns `Some("__lp_q32_lpfn_snoise3")` for simplex
...
2. For each variant, look for matching function name (e.g., `__lp_q32_lpfn_snoise3` for simplex,
   `__lpfn_hash_*` for hash)
...
`__lp_float_lpfn_snoise3` (or similar) implementations, and the codegen will generate calls to those
instead of the q32 versions.

Actually, wait - if we're generating TestCase calls to `__lpfn_snoise3`, and there's no float
implementation yet, what happens? The transform will convert `__lpfn_snoise3` →
`__lp_q32_lpfn_snoise3`, which exists. So the current flow is:

- Codegen: `lpfn_snoise3(...)` → TestCase call to `__lpfn_snoise3`
- Transform: `__lpfn_snoise3` → `__lp_q32_lpfn_snoise3` (via mapping)
- Runtime: Calls `__lp_q32_lpfn_snoise3`

When float support is added:

- Codegen: `lpfn_snoise3(...)` → TestCase call to `__lpfn_snoise3` (same)
- Transform: If float target, keep as `__lp_float_lpfn_snoise3` (or similar)
- Runtime: Calls `__lp_float_lpfn_snoise3`

So the TestCase name `__lpfn_snoise3` represents the "semantic" function, and the transform decides
...
2. **Codegen** (`lp_lib_fns.rs`):
    ...
    - Should emit TestCase calls using `LpLibFn::symbol_name()` (e.g., `"__lpfn_snoise3"`)

3. **Transform** (`calls.rs`):
    - `map_testcase_to_builtin()` should map `"__lpfn_snoise3"` → `BuiltinId::LpSimplex3`
    - `BuiltinId::LpSimplex3.name()` should return `"__lp_q32_lpfn_snoise3"` (actual function)
    - Transform looks up `"__lp_q32_lpfn_snoise3"` in `func_id_map`

4. **Hash Functions**:
    - In `builtins/shared/` (correct - not float-specific)
    - Use `__lpfn_hash_*` naming (no `q32` prefix)
...
- Semantic function names (`lpfn_snoise3`)
- TestCase names (`__lpfn_snoise3`) via `symbol_name()`
...
- Generate `BuiltinId` variants matching `LpLibFn::builtin_id()` (e.g., `LpSimplex3`, not
  `Q32LpSimplex3`)
- Map `BuiltinId::LpSimplex3.name()` to actual function name (`__lp_q32_lpfn_snoise3`)

**Flow**:

1. Codegen: `lpfn_snoise3(...)` → TestCase call to `LpLibFn::Simplex3.symbol_name()` =
   `"__lpfn_snoise3"`
2. Transform: `"__lpfn_snoise3"` → `BuiltinId::LpSimplex3` (via `map_testcase_to_builtin()`)
3. Transform: `BuiltinId::LpSimplex3.name()` → `"__lp_q32_lpfn_snoise3"`
4. Transform: Look up `"__lp_q32_lpfn_snoise3"` in `func_id_map` → create call
5. Runtime: Calls `__lp_q32_lpfn_snoise3`
...
1. **Fix Generator** - Update `lps-builtin-gen-app` to use `LpLibFn` enum as source of truth:
    ...
    - Generate registry with actual function names (e.g., `__lp_q32_lpfn_snoise3`)

2. **Fix Codegen** - Change `emit_lp_lib_fn_call()` to emit TestCase calls:
    - Use `LpLibFn::symbol_name()` to get TestCase name (e.g., `"__lpfn_snoise3"`)
...
3. **Verify Transform** - Ensure `map_testcase_to_builtin()` correctly maps:
    - `"__lpfn_snoise3"` → `BuiltinId::LpSimplex3`
    - Transform uses `BuiltinId::LpSimplex3.name()` → `"__lp_q32_lpfn_snoise3"`
    - Looks up `"__lp_q32_lpfn_snoise3"` in `func_id_map`

4. **Test** - Verify end-to-end flow:
    - Codegen emits TestCase call to `"__lpfn_snoise3"`
    - Transform converts to `__lp_q32_lpfn_snoise3` call
