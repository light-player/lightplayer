...
Check `lp-shader/lps-compiler/src/backend/transform/q32/converters/math.rs`:

- `map_testcase_to_builtin("__lpfn_snoise1")` should return `Some((BuiltinId::LpSimplex1, 2))`
- `map_testcase_to_builtin("__lpfn_snoise2")` should return `Some((BuiltinId::LpSimplex2, 3))`
- `map_testcase_to_builtin("__lpfn_snoise3")` should return `Some((BuiltinId::LpSimplex3, 4))`
- Hash functions should not be in the mapping (they don't go through transform)

### 5.2 Verify Transform Conversion Logic

Check `lp-shader/lps-compiler/src/backend/transform/q32/converters/calls.rs`:

- `convert_call()` should detect TestCase calls to `"__lpfn_snoise3"`
- Should map to `BuiltinId::LpSimplex3` via `map_testcase_to_builtin()`
- Should use `BuiltinId::LpSimplex3.name()` to get `"__lp_q32_lpfn_snoise3"`
- Should look up `"__lp_q32_lpfn_snoise3"` in `func_id_map`
