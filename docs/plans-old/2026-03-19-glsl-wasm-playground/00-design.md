...
Build a browser-based demo that compiles GLSL to WASM and renders shader output to a canvas. The
GLSL compiler itself runs as WASM in the browser (no server). Target: rainbow.shader renders in a
browser.
...
lp-app/
└── web-demo/
    ├── Cargo.toml              # NEW: wasm-pack cdylib, deps: lps-frontend, lps-wasm, wasm-bindgen
    ├── src/
...
scripts/
└── build-builtins.sh           # EXISTS: already builds lps_builtins_wasm.wasm
...
Dependencies: `lps-frontend`, `lps-wasm`, `wasm-bindgen`. No Cranelift.
...
- **Error panel**: Shows compilation errors. While user edits, keeps rendering last successful
  shader.
- **JS glue**: Loads compiler WASM (wasm-pack output) and builtins WASM (fetched as separate file).
  Handles compile → instantiate → render loop.
