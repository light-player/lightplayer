...
9. **Cleanup** — Removed `filetests/wasm/` directory. Fixed fn-equal generator: corrected D2 expected value for `in_expression` test (`equal(equal(a,b), equal(b,c))` = (false, true)). Fixed clippy warnings in lps-filetests.
