...
3. If yes, use `LpfnFn` → `q32_impl` lookup
...
1. Update `lp-shader/lps-compiler/src/backend/transform/q32/converters/calls.rs`:
    - In `convert_call`, when handling TestCase names:
        ...
        - If `Some(builtin_id)`, call `find_lpfn_fn_by_builtin_id(builtin_id)`
        - If `Some(lpfn_fn)`, extract `q32_impl` from `lpfn_fn.impls`
...
2. Import necessary functions:
    ...
    - `use crate::frontend::semantic::lpfn::lpfn_fn_registry::find_lpfn_fn_by_builtin_id;`
