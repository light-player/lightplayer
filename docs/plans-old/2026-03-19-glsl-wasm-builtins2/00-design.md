...
- **Fix psrdnoise seed bug:** Add `seed: UInt` to `lpfn_psrdnoise` GLSL signatures (vec2 and vec3
...
- **LPFX call emission:** New `lpfn_call.rs` for LPFX FunCall dispatch — arg flattening, out-param
...
| LPFX codegen     | New `lpfn_call.rs`, separate from `builtin_call.rs` (match Cranelift split) |
...
├── lps-frontend/
│   └── src/semantic/lpfn/
│       └── lpfn_fns.rs                          # UPDATE: add seed param to psrdnoise sigs
├── lps-cranelift/
...
│       ├── expr/
│       │   ├── mod.rs                            # UPDATE: add is_lpfn_fn dispatch branch
│       │   ├── builtin_inline.rs                 # UPDATE: add floor, fract Q32
│       │   ├── builtin_call.rs                   # (no changes)
│       │   └── lpfn_call.rs                      # NEW: LPFX call emission + arg flatten + out
...
├── lps-builtins-gen-app/                     # UPDATE: regenerate after lpfn_fns.rs change
examples/
...
FunCall(name, args)
  ...
  ├─ LPFX function              →  lpfn_call.rs       (worley, fbm, psrdnoise)
  └─ error
```

## LPFX call emission (`lpfn_call.rs`)

1. `find_lpfn_fn(name, &arg_types)` → get GLSL signature + `BuiltinId`
...
| `lpfn_call.rs`      | LPFX dispatch: arg flatten, out-param pointers, post-call loads  |
...
| `lpfn_fns.rs`       | Fix: add `seed: UInt` to psrdnoise GLSL sigs                     |
