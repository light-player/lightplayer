...
**`lpvm_cranelift::JitModule`** (Q32 primary).
...
- **Errors:** map **`lpvm_cranelift::CompilerError`** / **`CallError`** into
  **`lps_diagnostics::GlslError`** with usable messages (pattern after
  **`WasmExecutable`**).
- **`format_emulator_state` / CLIF / VCode:** return `None` / empty for JIT
  unless cheap to forward from `lpvm-cranelift` later.

## Tests

- Unit tests with **minimal GLSL** compiled through **`lpvm_cranelift::jit`**:
