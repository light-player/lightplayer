...
The code is doing direct string checks against function names instead of using `LpfxFnId` which should be the single source of truth. This is brittle and error-prone.
...
**Context**: Currently hash functions map to testcase names like `"1f" | "__lp_1"`. Should this be derived from `LpfxFnId` or kept as-is?

**Suggestion**: Add a method `testcase_names()` that returns all valid testcase names for a function.
