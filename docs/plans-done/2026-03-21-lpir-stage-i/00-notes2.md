...
2. **GLSL version target**: The pipeline uses **GLSL 4.50 core** (`#version
   450 core` in `lps-frontend`). GLSL 4.50 allows recursion (only GLSL ES
...
- [x] Naga coverage matrix — §1 direction set (arrays + in/out in scope, textures/atomics deferred);
  full table deferred to `08-glsl-mapping.md`
