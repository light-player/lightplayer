...
The guest code currently lives in `lp-shader/lps-builtins-emu-app/src/`:
...
- `lps-builtins` crate - Provides builtin functions that need to be referenced
...
- `lps-builtins-emu-app` currently uses this code as its main binary
- `lps-compiler` builds `lps-builtins-emu-app` and embeds it as a library
...
- `lps-builtins-emu-app` needs to use this code as its main binary
...
- The binary (`lps-builtins-emu-app`) will become a thin wrapper that calls these functions

**Answer**: Agreed. `lp-riscv-emu-guest` will be a library crate with public modules and functions.
`lps-builtins-emu-app` becomes a thin wrapper.
...
application (`lps-builtins-emu-app`)?
...
- `builtin_refs.rs` is auto-generated by `lps-builtin-gen-app`
- It references all builtin functions to prevent dead code elimination
- `lps-builtins-emu-app` needs this to ensure builtins are included
...
`builtin_refs` module if they need custom behavior. For `lps-builtins-emu-app`, we'll use the
default.

**Answer**: Builtin references are app-specific. `lp-riscv-emu-guest` will NOT include
`builtin_refs.rs`. Each application (`lps-builtins-emu-app`, firmware, etc.) will provide its
own
...
### Q6: lps-builtins-emu-app Refactoring

**Question**: After extraction, what should `lps-builtins-emu-app` become? A thin binary
wrapper, or
should it be removed entirely?

**Context**:

- `lps-builtins-emu-app` is currently built and embedded by `lps-compiler`
- It serves as a way to link all builtins into a static library
- The binary needs to call `_lp_main()` which references builtins

**Suggested Answer**: `lps-builtins-emu-app` becomes a thin binary that:
...
**Answer**: Agreed. `lps-builtins-emu-app` will become a thin binary wrapper that depends on
