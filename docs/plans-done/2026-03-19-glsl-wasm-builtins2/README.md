...
**Predecessor:** `docs/plans-done/2026-03-18-glsl-wasm-builtins/` — `builtins.wasm`, Q32 import plumbing, inline gentype builtins, `q32_builtin_link` smoke test.

**Goal:** `examples/basic/src/rainbow.shader/main.glsl` compiles with `glsl_wasm` Q32, links with `lp_glsl_builtins_wasm.wasm` + shared `env.memory`, and runs under wasmtime.
...
Design references (archived): `docs/plans-done/2026-03-18-glsl-wasm-builtins/00-design.md`, `00-notes.md`.
