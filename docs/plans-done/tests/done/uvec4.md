...
Create a complete test suite for unsigned integer vector type `uvec4` in `lightplayer/crates/lps-filetests/filetests/uvec4/` following the organizational pattern used in `vec4/` and `bool/`. These tests will comprehensively cover the GLSL unsigned integer vector specification for `uvec4` and are expected to fail initially, serving as a specification for implementing the rest of the compiler.
