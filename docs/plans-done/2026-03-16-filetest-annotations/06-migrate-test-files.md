...
For every `.glsl` file in `lp-glsl/lp-glsl-filetests/filetests/` (excluding
...
```bash
grep -r '// target' lp-glsl/lp-glsl-filetests/filetests/ --include='*.glsl' | grep -v '.gen.glsl'
grep -r '\[expect-fail\]' lp-glsl/lp-glsl-filetests/filetests/ --include='*.glsl' | grep -v '.gen.glsl'
```
