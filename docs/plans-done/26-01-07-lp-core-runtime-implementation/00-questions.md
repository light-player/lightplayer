...
6. **Shader Node Runtime** - Implement `nodes/shader/runtime.rs` and `nodes/shader/config.rs` (uses `GlslExecutable` from `lps-compiler` crate) (+ tests)
...
**Note**: `GlslExecutable` trait already exists in `lps-compiler` crate, so we can use that directly.
