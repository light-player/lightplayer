...
Remove the `max_instructions` field from `Riscv32Emulator` state and all related methods (
`with_max_instructions()`, `set_max_instructions()`). Update any code that references these.
...
Update call sites to use `run_fuel()` instead, or remove the calls if they're setting a global limit
...
The `EmulatorError::InstructionLimitExceeded` variant might still be used by `run_until_yield()`
when fuel is exhausted. That's fine - we can keep the error variant, it's just that the limit is now
per-run instead of global.

Actually, `run_until_yield()` returns `InstructionLimitExceeded` when fuel is exhausted, which is
correct. The error still makes sense, just the semantics changed (per-run instead of global).
...
- No references to `with_max_instructions()` or `set_max_instructions()` remain (except in error
  messages or comments if needed)
