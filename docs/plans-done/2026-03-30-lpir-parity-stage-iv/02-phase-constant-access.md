...
In `lp-shader/lps-frontend/src/naga_util.rs`, `expr_type_inner()` already handles `AccessIndex` on
...
In `lp-shader/lps-frontend/src/lower_expr.rs`, extend the `AccessIndex` match arm:
...
In `lp-shader/lps-frontend/src/lower_stmt.rs`, handle `Store` where the pointer is an array element
access:
