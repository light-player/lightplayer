...
| [`lp-glsl-core`](lp-glsl-core/) | Shared GLSL type / function-signature shapes (no parser). |
| [`lp-glsl-diagnostics`](lp-glsl-diagnostics/) | `GlslError`, codes, source locations. |
| [`lp-glsl-values`](lp-glsl-values/) | Runtime values and literal parsing (`glsl` fork). |
...
**Dependency spine (firmware):** `lp-glsl-naga` → `lpir` ← `lpir-cranelift` ← `lp-engine`; `lp-glsl-builtins` + `lp-glsl-builtin-ids` alongside codegen.

**Test-only / host helpers:** `lp-glsl-exec`, `lp-glsl-values`, `lp-glsl-core`, `lp-glsl-diagnostics`, `lp-glsl-filetests*`, `lp-glsl-wasm` (as used by filetests and web demo).
