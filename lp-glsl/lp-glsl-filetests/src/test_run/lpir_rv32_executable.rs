//! [`lp_glsl_exec::GlslExecutable`] via RV32 object + linked builtins + emulator.
...
impl LpirRv32Executable {
    ...
    pub fn try_new(
        source: &str,
        float_mode: crate::target::FloatMode,
    ) -> Result<Self, CompilerError> {
        let naga =
            lp_glsl_naga::compile(source).map_err(|e| CompilerError::Parse(format!("{e}")))?;
...
impl Q32ShaderExecutable for LpirRv32Executable {
    fn call_q32_ret(
        ...
        let gfn = self.gfn_meta(name).ok_or_else(|| {
            GlslError::new(
                lp_glsl_diagnostics::ErrorCode::E0101,
