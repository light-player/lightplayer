...
Tool for tracking q32 transform bloat metrics by generating detailed before/after reports for GLSL test files.
...
```bash
./scripts/q32-metrics.sh
```

Or run directly:

```bash
cargo run --bin q32-metrics -- \
