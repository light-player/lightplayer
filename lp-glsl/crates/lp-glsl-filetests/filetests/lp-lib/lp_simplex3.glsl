...
// ============================================================================
// lp_simplex3(): 3D Simplex noise function
// ============================================================================

float test_lp_simplex3_basic() {
    // Test basic 3D simplex noise - should be in [-1, 1] range
    vec3 p = vec3(0.5, 0.5, 0.5);
    uint seed = 0u;
    float n = lp_simplex3(p, seed);
    return (n >= -1.0 && n <= 1.0) ? 1.0 : 0.0;
}

// run: test_lp_simplex3_basic() == 1.0

float test_lp_simplex3_origin() {
...
// run: test_lp_simplex3_origin() == 1.0

float test_lp_simplex3_deterministic() {
    // Same inputs should produce same output
    vec3 p = vec3(0.5, 0.5, 0.5);
    float n1 = lp_simplex3(p, 0u);
    float n2 = lp_simplex3(p, 0u);
    return abs(n1 - n2);
}

// run: test_lp_simplex3_deterministic() ~= 0.0
...
float test_lp_simplex3_range() {
    // Test multiple values to ensure they're in valid range
    float n1 = lp_simplex3(vec3(0.0, 0.0, 0.0), 0u);
    float n2 = lp_simplex3(vec3(1.0, 0.0, 0.0), 0u);
    float n3 = lp_simplex3(vec3(0.0, 1.0, 0.0), 0u);
    float n4 = lp_simplex3(vec3(0.0, 0.0, 1.0), 0u);
    float n5 = lp_simplex3(vec3(10.0, 10.0, 10.0), 0u);
...
// run: test_lp_simplex3_range() == 1.0
