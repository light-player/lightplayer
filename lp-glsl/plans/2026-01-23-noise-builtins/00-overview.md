...
- Hash function (`lp_hash`) with 1D, 2D, and 3D variants
- Simplex noise functions (`lp_simplex1`, `lp_simplex2`, `lp_simplex3`)
- Integration with the existing builtin system
- Semantic checking and codegen for user-facing `lp_*` function names

## Phases

1. **Implement hash function** - Create `lp_hash.rs` with Q32 fixed-point hash functions using noiz algorithm
2. **Implement Simplex noise functions** - Create `lp_simplex1.rs`, `lp_simplex2.rs`, `lp_simplex3.rs` with Q32 implementations
