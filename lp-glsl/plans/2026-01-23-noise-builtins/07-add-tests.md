...
Create tests comparing LP library function outputs against noise-rs reference implementation. Add noise-rs as a test-only dependency and create comparison tests.

## Implementation

### Add Test Dependency

Update `lp-builtins/Cargo.toml`:
```toml
[dev-dependencies]
noise = "0.9"
```
...
3. **`lp_simplex2.rs` tests**
   - Compare against noise-rs Simplex 2D
   - Test output range
   - Test continuity

4. **`lp_simplex3.rs` tests**
