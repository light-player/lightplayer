...
Create the hash function implementation in `lp-builtins/src/builtins/fixed32/lp_hash.rs` with three variants for 1D, 2D, and 3D hashing. The hash function uses the noiz algorithm optimized for noise generation.

## Implementation

### File: `lp-builtins/src/builtins/fixed32/lp_hash.rs`

Implement three hash functions:
- `__lp_hash_1(x: u32, seed: u32) -> u32` - 1D hash
- `__lp_hash_2(x: u32, y: u32, seed: u32) -> u32` - 2D hash  
- `__lp_hash_3(x: u32, y: u32, z: u32, seed: u32) -> u32` - 3D hash
...
For multi-dimensional inputs, combine coordinates non-commutatively before hashing (similar to noiz's `NoiseRngInput` trait implementations).
