...
1. **Extend LpLibFn with q32 mapping methods** - Add `needs_q32_mapping()` and `q32_name()` methods to determine conversion requirements
2. **Update codegen to emit TestCase calls** - Change `emit_lp_lib_fn_call()` to emit TestCase calls for functions that need q32 mapping
...
4. **Regenerate builtin registry** - Run generator to create correct `BuiltinId` variants (LpSimplex3, not Q32LpSimplex3)
