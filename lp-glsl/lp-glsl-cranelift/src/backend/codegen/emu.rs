...
pub struct EmulatorOptions {
    ...
    /// Log level for emulator execution (None for no logging)
    pub log_level: lp_riscv_emu::LogLevel,
}
...
pub fn build_emu_executable(
    ...
    // Fuel is now per-run, not global, so we don't need to set limits here
...
mod tests {
    ...
    fn test_build_emu_executable() {
        ...
        let mut gl_module = GlModule::new_object(target, crate::FloatMode::Q32).unwrap();
