...
use super::glsl_types::glsl_type_to_cranelift;
            ...
            ParamQualifier::In => {
                if ty.is_array() {
                    ...
                    let cranelift_ty = if base_ty == Type::Float {
                        ...
                        glsl_type_to_cranelift(&base_ty)
                    ...
                    let cranelift_ty = if *ty == Type::Float {
                        ...
                        glsl_type_to_cranelift(&ty).expect("scalar type should be convertible")
                    };
                    sig.params.push(AbiParam::new(cranelift_ty));
                }
            }
    ...
    fn add_type_as_returns(
        ...
        if ty.is_vector() {
            ...
            let cranelift_ty = if *ty == Type::Float {
                ...
                glsl_type_to_cranelift(&ty).expect("scalar return type should be convertible")
            };
