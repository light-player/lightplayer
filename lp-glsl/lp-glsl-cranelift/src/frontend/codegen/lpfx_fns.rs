...
use super::lpfx_sig::build_call_signature;
use crate::FloatMode;
...
impl<'a, M: cranelift_module::Module> CodegenContext<'a, M> {
    ...
    pub fn emit_lp_lib_fn_call(
        ...
        match &func.impls {
            lp_glsl_frontend::semantic::lpfx::lpfx_fn::LpfxFnImpl::Decimal {
                float_impl,
                q32_impl,
            } => {
                ...
                if let Some(buffer_ptr) = return_buffer_ptr {
                    ...
                    let cranelift_ty = if self.is_q32() && base_type == Type::Float {
                        ...
                        crate::frontend::codegen::glsl_types::glsl_type_to_cranelift(&base_type)
                            .map_err(|e| {
                                GlslError::new(
                                    ErrorCode::E0400,
                                    format!(
            ...
            lp_glsl_frontend::semantic::lpfx::lpfx_fn::LpfxFnImpl::NonDecimal(builtin_id) => {
    ...
    fn get_lpfx_testcase_call(
        &mut self,
        func: &'static lp_glsl_frontend::semantic::lpfx::lpfx_fn::LpfxFn,
