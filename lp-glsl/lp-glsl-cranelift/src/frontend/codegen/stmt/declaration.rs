...
use lp_glsl_frontend::semantic::const_eval;
...
pub fn emit_declaration<M: cranelift_module::Module>(
    ...
    match decl {
        Declaration::InitDeclaratorList(list) => {
            ...
            let mut const_env_buf = HashMap::new();
            ctx.fill_const_env(&mut const_env_buf);
            let const_env = Some(&const_env_buf);

            // Handle the head declaration
            if let Some(name) = &list.head.name {
                let mut ty = lp_glsl_frontend::semantic::type_resolver::parse_head_declarator_type(
                    list, &name.span, const_env,
                )?;
                    ...
                    if let Some(&first_dim) = array_dims.first() {
                        if first_dim == 0 {
                            ...
                            if let Some(init) = &list.head.initializer {
                                ...
                                ty = lp_glsl_frontend::semantic::types::Type::Array(
                                    Box::new(element_ty),
                                    inferred_size,
                                );
                            } else {
                    ...
                    if let Some(init) = &list.head.initializer {
                        if let glsl::syntax::Initializer::Simple(expr) = init {
                            ...
                            if let Ok(val) = const_eval::eval_constant_expr(
                                ...
                                ctx.add_local_const(name.name.clone(), val);
                            }
                        }
                    }
                } else {
                    let _ = ctx.declare_variable(name.name.clone(), ty.clone())?;
                }

                // Handle initializer if present (non-const or array init)
                if !is_const && ty.is_array() {
                    if let Some(init) = &list.head.initializer {
                        ...
                        lp_glsl_frontend::semantic::type_check::check_assignment(
                            &element_ty,
                            &init_element_ty,
                        )
                        .map_err(|_e| {
                        ...
                        if init_vals.len() < array_size {
                            let zero_val = match element_ty {
                                lp_glsl_frontend::semantic::types::Type::Int => ctx
                                    .builder
                                    .ins()
                                    .iconst(cranelift_codegen::ir::types::I32, 0),
                                lp_glsl_frontend::semantic::types::Type::Float => {
                                    ctx.emit_float_const(0.0)
                                }
                                lp_glsl_frontend::semantic::types::Type::Bool => ctx
                                    .builder
                                    .ins()
                                    .iconst(cranelift_codegen::ir::types::I32, 0),
                                _ => {
                    ...
                    match lp_glsl_frontend::semantic::type_check::check_assignment(&ty, &init_ty) {
                        Ok(()) => {}
                        Err(mut error) => {
            ...
            for declarator in &list.tail {
                let mut declarator_ty =
                    lp_glsl_frontend::semantic::type_resolver::parse_tail_declarator_type(
                        &base_ty, declarator, const_env,
                    )?;
                    ...
                    if let Some(&first_dim) = array_dims.first() {
                        if first_dim == 0 {
                            ...
                            if let Some(init) = &declarator.initializer {
                                ...
                                declarator_ty = lp_glsl_frontend::semantic::types::Type::Array(
                                    Box::new(element_ty),
                                    inferred_size,
                                );
                            } else {
                    ...
                    if let Some(init) = &declarator.initializer {
                        if let glsl::syntax::Initializer::Simple(expr) = init {
                            ...
                            if let Ok(val) = const_eval::eval_constant_expr(
                                ...
                                ctx.add_local_const(declarator.ident.ident.name.clone(), val);
                            }
                        }
                    }
                } else {
                    let _vars = ctx.declare_variable(
                        declarator.ident.ident.name.clone(),
                        declarator_ty.clone(),
                    )?;

                    // Handle initializer if present (non-const)
                    if declarator_ty.is_array() {
                        if let Some(init) = &declarator.initializer {
                            // Ensure we're in a block before emitting instructions
                            ctx.ensure_block()?;
                        ...
                        let vars = ctx
                            .lookup_variables(&declarator.ident.ident.name)
                            .expect("just declared")
                            .to_vec();
                        if vars.len() != init_vals.len() {
                            return Err(GlslError::new(
...
) -> Result<lp_glsl_frontend::semantic::types::Type, GlslError> {
    // Use unified type parser from type_resolver.rs which handles arrays
    lp_glsl_frontend::semantic::type_resolver::parse_type_specifier(&type_spec.ty, None)
}
...
> {
    ...
    match init {
        ...
        Initializer::List(list) => {
            ...
            let mut element_type: Option<lp_glsl_frontend::semantic::types::Type> = None;
            ...
            for item in list.0.iter() {
                ...
                if let Some(ref expected_ty) = element_type {
                    ...
                    lp_glsl_frontend::semantic::type_check::check_assignment(expected_ty, &item_ty)
                        .map_err(|_e| {
                            GlslError::new(
                                ErrorCode::E0400,
                                format!(
