...
pub fn emit_condition<M: cranelift_module::Module>(
    ...
    match condition {
        glsl::syntax::Condition::Expr(expr) => {
            ...
            match lp_glsl_frontend::semantic::type_check::check_condition(&ty) {
                Ok(()) => {}
                Err(mut error) => {
        ...
        glsl::syntax::Condition::Assignment(type_spec, identifier, initializer) => {
            ...
            match lp_glsl_frontend::semantic::type_check::check_assignment(&var_ty, &init_ty) {
                Ok(()) => {}
                Err(mut error) => {
                    if error.location.is_none() {
                        error =
            ...
            match lp_glsl_frontend::semantic::type_check::check_condition(&var_ty) {
                Ok(()) => {}
                Err(mut error) => {
                    if error.location.is_none() {
                        error =
