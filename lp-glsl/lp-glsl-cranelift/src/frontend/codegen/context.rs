...
use lp_glsl_frontend::src_loc::{GlFileId, GlSourceMap};
...
pub struct CodegenContext<'a, M: Module> {
    ...
    pub global_constants:
        Option<&'a hashbrown::HashMap<String, lp_glsl_frontend::semantic::const_eval::ConstValue>>,

    // Local const values declared in current function (for array size resolution).
    pub local_const_env:
        hashbrown::HashMap<String, lp_glsl_frontend::semantic::const_eval::ConstValue>,
...
impl<'a, M: Module> CodegenContext<'a, M> {
    ...
    pub fn fill_const_env(
        &self,
        out: &mut hashbrown::HashMap<String, lp_glsl_frontend::semantic::const_eval::ConstValue>,
    ) {
        out.clear();
    ...
    pub fn add_local_const(
        &mut self,
        name: String,
        val: lp_glsl_frontend::semantic::const_eval::ConstValue,
    ) {
    ...
    pub fn add_span_to_error(
        ...
        let location = lp_glsl_frontend::src_loc::GlSourceLoc::new(
    ...
    pub fn calculate_array_element_size_bytes(
        ...
        if element_ty.is_vector() {
            ...
            let base_cranelift_ty =
                super::glsl_types::glsl_type_to_cranelift(&base_ty).map_err(|e| {
            ...
            // Scalar: use glsl_type_to_cranelift
            let element_cranelift_ty = super::glsl_types::glsl_type_to_cranelift(&element_ty)
                .map_err(|e| {
                    crate::error::GlslError::new(
                        crate::error::ErrorCode::E0400,
                        format!(
