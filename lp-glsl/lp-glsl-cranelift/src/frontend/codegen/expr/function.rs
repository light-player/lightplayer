...
pub fn emit_function_call<M: cranelift_module::Module>(
    ...
    if lp_glsl_frontend::semantic::builtins::is_builtin_function(func_name) {
        return emit_builtin_call_expr(ctx, func_name, args, span.clone());
    }

    // Check if it's an LPFX function
    if lp_glsl_frontend::semantic::lpfx::lpfx_fn_registry::is_lpfx_fn(func_name) {
...
fn emit_builtin_call_expr<M: cranelift_module::Module>(
    ...
    match lp_glsl_frontend::semantic::builtins::check_builtin_call(name, &arg_types) {
        Ok(_return_type) => {
...
fn emit_lp_lib_fn_call_expr<M: cranelift_module::Module>(
    ...
    let func = lp_glsl_frontend::semantic::lpfx::lpfx_fn_registry::find_lpfx_fn(name, &arg_types)
        .ok_or_else(|| {
        GlslError::new(
    ...
    match lp_glsl_frontend::semantic::lpfx::lpfx_fn_registry::check_lpfx_fn_call(name, &arg_types) {
        Ok(_return_type) => {
            // Validation passed, proceed with codegen
        }
        Err(err_msg) => {
...
fn lookup_function_signature<M: cranelift_module::Module>(
    ...
    (
        ...
        lp_glsl_frontend::semantic::functions::FunctionSignature,
    ),
    GlslError,
> {
    let func_ids = ctx
...
fn validate_function_call<M: cranelift_module::Module>(
    ctx: &mut CodegenContext<'_, M>,
    func_sig: &lp_glsl_frontend::semantic::functions::FunctionSignature,
    args: &[glsl::syntax::Expr],
    arg_types: &[GlslType],
    name: &str,
    call_span: &glsl::syntax::SourceSpan,
    ...
    for (param, arg_ty) in func_sig.parameters.iter().zip(arg_types) {
        ...
        if arg_base != param_base
            && !lp_glsl_frontend::semantic::type_check::can_implicitly_convert(
                &arg_base,
                &param_base,
            )
        {
    ...
    for arg_info in out_inout_args {
        ...
        let base_cranelift_ty = if arg_info.param_ty.is_vector() {
            ...
            if base == crate::semantic::types::Type::Float {
                ...
                crate::frontend::codegen::glsl_types::glsl_type_to_cranelift(&base).map_err(
                    |e| {
                        GlslError::new(
                            ErrorCode::E0400,
            ...
            crate::frontend::codegen::glsl_types::glsl_type_to_cranelift(&arg_info.param_ty)
                .map_err(|e| {
                    GlslError::new(
                        ErrorCode::E0400,
                        format!("Failed to convert type: {}", e.message),
...
fn execute_function_call<M: cranelift_module::Module>(
    ...
    if let Some(buffer_ptr) = return_buffer_ptr {
        ...
        let cranelift_ty = if base_type == crate::semantic::types::Type::Float {
            ...
            crate::frontend::codegen::glsl_types::glsl_type_to_cranelift(&base_type).map_err(
                |e| {
                    GlslError::new(
                        ErrorCode::E0400,
                        format!(
