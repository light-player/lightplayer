...
use lp_glsl_frontend::src_loc::GlSourceMap;
...
pub struct GlslEmulatorModule {
    ...
    pub(crate) source_loc_manager: lp_glsl_frontend::src_loc_manager::SourceLocManager,
...
impl GlslEmulatorModule {
    ...
    fn get_main_file_id(&self) -> Option<lp_glsl_frontend::src_loc::GlFileId> {
        ...
        let main_file_id = lp_glsl_frontend::src_loc::GlFileId(1);
        if self.source_map.get_file(main_file_id).is_some() {
            ...
            for i in 1..=10 {
                let file_id = lp_glsl_frontend::src_loc::GlFileId(i);
    ...
    fn format_source_lines_around_trap(
        &self,
        file_id: lp_glsl_frontend::src_loc::GlFileId,
        trap_line: usize,
        trap_column: usize,
    ) -> Option<String> {
    ...
    fn format_trap_error_from_emulator_error(
        ...
        let file_id = self.get_main_file_id().unwrap_or_else(|| {
            lp_glsl_frontend::src_loc::GlFileId(1) // Fallback to ID 1 if helper fails
        });

        if let Some((trap_line, trap_column)) = self.source_loc_manager.lookup_srcloc(
            crate::frontend::codegen::srcloc::from_cranelift_srcloc(srcloc),
        ) {
            // Found exact match
            let trap_location =
                lp_glsl_frontend::src_loc::GlSourceLoc::new(file_id, trap_line, trap_column);
        ...
        if found_location.is_none() {
                ...
                .min_by_key(|(trap_pc, _, _, _)| (*trap_pc as i64 - pc as i64).abs())
            {
                if !closest_srcloc.is_default() {
                    if let Some((trap_line, trap_column)) = self.source_loc_manager.lookup_srcloc(
                        crate::frontend::codegen::srcloc::from_cranelift_srcloc(*closest_srcloc),
                    ) {
                        let trap_location = lp_glsl_frontend::src_loc::GlSourceLoc::new(
                            file_id,
                            trap_line,
                            trap_column,
                        );
    ...
    fn find_function_source_location(
        &self,
        func_name: &str,
    ) -> Option<lp_glsl_frontend::src_loc::GlSourceLoc> {
        ...
        for (line_idx, line) in source_text.lines().enumerate() {
            if line.contains(&pattern) {
                ...
                return Some(lp_glsl_frontend::src_loc::GlSourceLoc::new(
                    file_id,
                    line_idx + 1,
                    1,
                ));
