# lp-glsl-cranelift - GLSL Compiler (Cranelift Backend)
...
```rust
use lp_glsl_cranelift::Compiler;
...
Run the example:
```bash
cargo run -p lp-glsl-cranelift --example simple
```
...
```bash
# Run all tests
cargo test -p lp-glsl-cranelift
...
Uses `lp-glsl-frontend` for parsing, semantic analysis, and type checking. This crate provides:

- `lib.rs` - Main entry point, `#![no_std]` with std feature gate
- `backend/` - Cranelift codegen, builtin registry, target abstractions
- `codegen/` - Cranelift IR code generation (expr, stmt, builtins)
- `exec/` - JIT and emulator execution
- `transform/` - AST transformations (fixed-point, Q32)
