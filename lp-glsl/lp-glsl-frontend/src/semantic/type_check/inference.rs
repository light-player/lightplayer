...
use crate::semantic::scope::{StorageClass, SymbolTable};
...
pub fn infer_expr_type_with_registry(
    ...
    match expr {
        ...
        Expr::FunCall(func_ident, args, span) => {
            ...
            else if crate::semantic::lpfx::lpfx_fn_registry::is_lpfx_fn(func_name) {
                match crate::semantic::lpfx::lpfx_fn_registry::check_lpfx_fn_call(
                    func_name, &arg_types,
                ) {
                    Ok(return_type) => Ok(return_type),
                    Err(err_msg) => Err(GlslError::new(ErrorCode::E0114, err_msg)
