...
use crate::legacy::nodes::{FixtureRuntime, OutputRuntime, ShaderRuntime, TextureRuntime};
use crate::legacy_project::LegacyProjectRuntime;
use crate::legacy_project::project_runtime::{NodeEntry, NodeStatus};
use crate::nodes::LegacyNodeRuntime;
use crate::output::OutputProvider;
use crate::panic_node::catch_node_panic;
                ...
                NodeKind::Texture => {
                    ...
                    if let Some(runtime) = &entry.runtime {
                        ...
                        if let Some(tex_runtime) = runtime.as_any().downcast_ref::<TextureRuntime>()
                        {
                            ...
                            NodeState::Texture(lpc_wire::legacy::nodes::texture::TextureState::new(
                                rt.frame_id,
                            ))
                        }
                    } else {
                ...
                NodeKind::Shader => {
                    ...
                    if let Some(runtime) = &entry.runtime {
                            ...
                            runtime.as_any().downcast_ref::<ShaderRuntime>()
                        {
                            ...
                            // Fallback to empty state
                            NodeState::Shader(lpc_wire::legacy::nodes::shader::ShaderState::new(
                                rt.frame_id,
                            ))
                        }
                    } else {
                ...
                NodeKind::Output => {
                    ...
                    if let Some(runtime) = &entry.runtime {
                        if let Some(output_runtime) = runtime
                            .as_any()
                            .downcast_ref::<crate::legacy::nodes::OutputRuntime>(
                        ) {
                ...
                NodeKind::Fixture => {
                    ...
                    if let Some(runtime) = &entry.runtime {
                            ...
                            runtime.as_any().downcast_ref::<FixtureRuntime>()
                        {
                            ...
                            // Fallback to empty state
                            NodeState::Fixture(lpc_wire::legacy::nodes::fixture::FixtureState::new(
                                rt.frame_id,
                            ))
                        }
                    } else {
                    ...
                    if let Some(runtime) = &entry.runtime {
                            ...
                            runtime.as_any().downcast_ref::<OutputRuntime>()
                        {
                            if let Some(output_config) = output_runtime.get_config() {
                                ...
                                Box::new(
                                    lpc_source::legacy::nodes::output::OutputConfig::GpioStrip {
                                        pin: 0,
                                        options: None,
                                    },
                                )
...
struct InitContext<'a> {
    runtime: &'a crate::legacy_project::LegacyProjectRuntime,
    #[allow(
        dead_code,
...
impl<'a> crate::runtime::contexts::NodeInitContext for InitContext<'a> {
    ...
    fn resolve_output(
        ...
        if entry.kind != lpc_source::legacy::nodes::NodeKind::Output {
            return Err(Error::WrongNodeKind {
                specifier: spec.as_str().to_string(),
                expected: lpc_source::legacy::nodes::NodeKind::Output,
                actual: entry.kind,
            });
        }
    ...
    fn resolve_texture(
        ...
        if entry.kind != lpc_source::legacy::nodes::NodeKind::Texture {
            return Err(Error::WrongNodeKind {
                specifier: spec.as_str().to_string(),
                expected: lpc_source::legacy::nodes::NodeKind::Texture,
                actual: entry.kind,
            });
        }
...
fn texture_output_owner_handle(
    nodes: &BTreeMap<lpc_model::NodeId, crate::legacy_project::project_runtime::NodeEntry>,
...
impl<'a> crate::runtime::contexts::RenderContext for RenderContextImpl<'a> {
    ...
    fn get_output(
        ...
        if let Some(runtime) = &mut entry.runtime {
            if let Some(output_runtime) = runtime
                .as_any_mut()
                .downcast_mut::<crate::legacy::nodes::OutputRuntime>()
            {
                ...
                && entry.runtime.is_some()
            {
                ...
                if let Some(runtime) = entry.runtime.as_ref() {
                    if let Some(shader_runtime) = runtime
                        .as_any()
                        .downcast_ref::<crate::legacy::nodes::ShaderRuntime>(
                    ) {
