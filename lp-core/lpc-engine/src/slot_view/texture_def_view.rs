...
use lpc_model::{Dim2u, SlotAccessorError, SlotShapeRegistry, SlotView, TextureDef};

/// Typed helper for reading texture definition fields through the resolver.
pub struct TextureDefView {
    inner: SlotView<TextureDef>,
}
...
impl TextureDefView {
    ...
    pub fn compile(registry: &SlotShapeRegistry) -> Result<Self, SlotAccessorError> {
        Ok(Self {
            inner: TextureDef::compile_slot_view(registry)?,
        })
    }
    ...
    pub fn size(&self, ctx: &mut TickContext<'_>) -> Result<Dim2u, NodeError> {
        ctx.resolve_consumed_slot_accessor_value(TextureDef::size(&self.inner))
    }
}
