...
//! represent); [`SrcSlot`]: a [`Shape`] plus **metadata** and wiring hints.
//!
...
//! projects cleanly to a [`crate::WireType`] and GPU layouts (`quantity.md` §6,
//! “Why no tuples”).
//!
//! **Defaults (M2, “Q15 Option A”):** [`SrcShape::Scalar`] carries a **mandatory**
//! [`ValueSpec`][`crate::value_spec::ValueSpec`]. [`SrcShape::Array`] and
//! [`SrcShape::Struct`] carry `default: Option<ValueSpec>`; if `None`, the
...
//! [`SrcSlot`] has **no** separate top-level `default` field: defaults are entirely
//! expressed through [`Shape`].
//!
//! [`SrcShape::Struct`]’s `fields` are a **vector** to preserve TOML order, std430
//! layout, and panel field order (`quantity.md` §6).

use crate::presentation::Presentation;
use crate::prop::src_binding::SrcBinding;
use crate::prop::src_value_spec::{FromTomlError, LoadCtx, SrcValueSpec};
use alloc::boxed::Box;
...
use lpc_model::prop::wire_type::{WireStructMember, WireType};
...
/// Omitted `shape` defaults to [`SrcShape::Scalar`]; in TOML output, `shape = "scalar"`
/// is elided (see the [`SrcSlot`] [`Serialize`][`serde::Serialize`] impl).
...
impl SrcSlot {
    /// Materialize the **default** for this slot: for [`SrcShape::Scalar`],
    /// [`SrcValueSpec::materialize`][`crate::value_spec::SrcValueSpec::materialize`]
    ...
    /// for compositions”.
    pub fn default_value(&self, ctx: &mut LoadCtx) -> WireValue {
        match &self.shape {
            SrcShape::Scalar { default, .. } => default.default_wire_value(ctx),
            SrcShape::Array {
                element,
                length,
                default,
            } => match default {
                Some(d) => d.default_wire_value(ctx),
    ...
    pub fn storage(&self) -> WireType {
        match &self.shape {
            ...
            SrcShape::Array {
                element, length, ..
            } => WireType::Array(Box::new(element.storage()), *length as usize),
            SrcShape::Struct { fields, .. } => WireType::Struct {
                name: None,
                fields: fields
                    .iter()
                    .map(|(name, slot)| WireStructMember {
                        name: name.0.clone(),
                        ty: slot.storage(),
                    })
                    .collect(),
...
/// for [`SrcSlot`] implement the TOML grammar in `docs/design/lightplayer/quantity.md` §10:
...
impl<'de> Deserialize<'de> for SrcSlot {
        ...
        D: Deserializer<'de>,
    {
        ...
        let table = value.as_table_mut().ok_or_else(|| {
            de::Error::custom("SrcSlot must deserialize to a TOML table (or JSON object)")
        })?;
        ...
        let mut table = value.as_table().cloned().ok_or_else(|| {
            de::Error::custom("internal: SrcSlot TOML must be a table after metadata extraction")
        })?;

        let shape = deserialize_shape_from_table(&mut table).map_err(D::Error::custom)?;
        if !table.is_empty() {
            return Err(de::Error::custom(alloc::format!(
                "unknown keys in SrcSlot table: {:?}",
...
impl SrcSlot {
    fn to_toml_table(&self) -> Result<toml::map::Map<String, toml::Value>, FromTomlError> {
        ...
        if matches!(self.shape, SrcShape::Scalar { .. }) {
            table.remove("shape");
        }
        ...
        if let Some(b) = &self.bind {
            ...
            match b {
                ...
                SrcBinding::Literal(_) | SrcBinding::NodeProp(_) => {
...
fn shape_to_toml_map(
    shape: &SrcShape,
) -> Result<toml::map::Map<String, toml::Value>, FromTomlError> {
    let mut t = toml::map::Map::new();
    match shape {
        SrcShape::Scalar {
            kind,
            ...
            t.insert(
                "default".into(),
                SrcValueSpec::to_toml_for_kind(default, *kind)?,
            );
        }
        SrcShape::Array {
            element,
            ...
            if let Some(d) = default {
                let s = SrcShape::Array {
                    element: element.clone(),
                    length: *length,
                    default: None,
                };
                t.insert("default".into(), SrcValueSpec::to_toml_for_shape(d, &s)?);
            }
        }
        SrcShape::Struct { fields, default } => {
...
fn slot_to_toml_map(s: &SrcSlot) -> Result<toml::map::Map<String, toml::Value>, FromTomlError> {
    s.to_toml_table()
}

fn slot_from_toml_value(v: toml::Value) -> Result<SrcSlot, String> {
...
fn deserialize_shape_from_table(
    ...
    match shape_s.as_str() {
        "scalar" => {
            ...
            let default = SrcValueSpec::from_toml_for_kind(&def, kind)
        ...
        "array" => {
            ...
            let default = match t.remove("default") {
                ...
                Some(v) => {
                    ...
                    Some(
                        SrcValueSpec::from_toml_for_shape(&v, &s)
                            .map_err(|e: FromTomlError| e.0.clone())?,
                    )
                }
            };
        ...
        "struct" => {
            ...
            let mut out: Vec<(NodeName, SrcSlot)> = Vec::with_capacity(a.len());
            for (i, item) in a.iter().enumerate() {
                ...
                let slot: SrcSlot = slot_from_toml_value(arr[1].clone())
...
impl schemars::JsonSchema for SrcSlot {
    fn schema_name() -> alloc::borrow::Cow<'static, str> {
        "SrcSlot".into()
    }

    /// Matched to [`impl Serialize for SrcSlot`]: a TOML `Table` (→ JSON object) with
    ...
    fn json_schema(generator: &mut schemars::SchemaGenerator) -> schemars::Schema {
        // `subschema_for::<SrcSlot>` first so the generator can emit a stable `$ref` for
        // recursive `array.element` and struct field slots.
        let slot = generator.subschema_for::<SrcSlot>();
        ...
        let bind = generator.subschema_for::<Option<SrcBinding>>();
...
mod tests {
    ...
    fn scalar_default_value_is_literal() {
        ...
        match scalar_amplitude_slot().default_value(&mut ctx) {
            WireValue::F32(v) => assert_eq!(v, 1.0),
    ...
    fn array_with_explicit_default_uses_override() {
        let elem = scalar_amplitude_slot();
        let preset: Vec<WireValue> = alloc::vec![WireValue::F32(0.2), WireValue::F32(0.7)];
        let array_slot = SrcSlot {
            shape: SrcShape::Array {
                element: Box::new(elem),
                length: 2,
                default: Some(SrcValueSpec::Literal(WireValue::Array(preset))),
            },
            label: None,
            description: None,
            bind: None,
        ...
        match array_slot.default_value(&mut ctx) {
            WireValue::Array(items) => {
                ...
                match (&items[0], &items[1]) {
                    (WireValue::F32(a), WireValue::F32(b)) => {
                        assert_eq!(*a, 0.2);
                        assert_eq!(*b, 0.7);
                    }
    ...
    fn slot_storage_projection_scalar() {
        assert_eq!(scalar_amplitude_slot().storage(), WireType::F32);
    }
    ...
    fn slot_storage_projection_array() {
        ...
        match array_slot.storage() {
            WireType::Array(element, len) => {
                assert_eq!(*element, WireType::F32);
                assert_eq!(len, 4);
            }
    ...
    fn slot_serde_round_trip_scalar() {
        ...
        let back: SrcSlot = serde_json::from_str(&json).unwrap();
        assert_eq!(s, back);
        let toml = toml::to_string(&s).unwrap();
        let back2: SrcSlot = toml::from_str(&toml).unwrap();
        assert_eq!(s, back2);
    }
    ...
    fn slot_serde_omits_none_overrides_on_composed() {
        ...
        let back: SrcSlot = toml::from_str(&toml).unwrap();
    ...
    fn scalar_slot_roundtrips_with_implicit_shape_in_toml() {
        ...
        let back: SrcSlot = toml::from_str(&s).unwrap();
        assert_eq!(slot, back);
    }
    ...
    fn scalar_slot_loads_when_shape_is_omitted() {
        ...
        let s: SrcSlot = toml::from_str(&written).unwrap();
    ...
    fn scalar_slot_loads_when_shape_is_explicit() {
        ...
        let s: SrcSlot = toml::from_str(&no_shape).unwrap();
        assert_eq!(s, expected);
    }
    ...
    fn color_slot_roundtrips_toml_css_string() {
        let c = WireValue::Struct {
            name: Some(String::from("Color")),
            fields: alloc::vec![
                (String::from("space"), WireValue::I32(0)),
                (String::from("coords"), WireValue::Vec3([0.7, 0.15, 90.0])),
            ],
        };
        let slot = SrcSlot {
            shape: SrcShape::Scalar {
                ...
                default: SrcValueSpec::Literal(c),
            },
            label: None,
            description: None,
            bind: None,
        ...
        let back: SrcSlot = toml::from_str(&t).unwrap();
        assert_eq!(slot, back);
    }

    #[test]
    fn audio_level_slot_roundtrips_toml() {
        let a = WireValue::Struct {
            name: Some(String::from("AudioLevel")),
            fields: alloc::vec![
                (String::from("low"), WireValue::F32(0.0)),
                (String::from("mid"), WireValue::F32(0.0)),
                (String::from("high"), WireValue::F32(0.0)),
            ],
        };
        let slot = SrcSlot {
            shape: SrcShape::Scalar {
                ...
                default: SrcValueSpec::Literal(a),
            },
            label: None,
            description: None,
            bind: None,
    ...
    fn bool_slot_roundtrips() {
        let slot = SrcSlot {
            shape: SrcShape::Scalar {
                ...
                default: SrcValueSpec::Literal(WireValue::Bool(true)),
            },
            label: None,
            description: None,
            bind: None,
    ...
    fn position2d_slot_roundtrips() {
        let slot = SrcSlot {
            shape: SrcShape::Scalar {
                ...
                default: SrcValueSpec::Literal(WireValue::Vec2([0.5, 0.5])),
            },
            label: None,
            description: None,
            bind: None,
    ...
    fn slot_metadata_fields_coexist_with_shape() {
        ...
        let expected = SrcSlot {
            label: Some(String::from("Speed")),
            description: Some(String::from("How fast")),
            bind: Some(SrcBinding::Bus(ChannelName(String::from(
                "audio/in/0/level",
            )))),
    ...
    fn array_with_props_at_root_is_rejected() {
        let toml = r#"
            ...
            default = 0.0
        "#;
        let res: Result<SrcSlot, _> = toml::from_str(toml);
        assert!(res.is_err());
    }
    ...
    fn slot_schema_is_non_degenerate() {
        let s = schemars::schema_for!(SrcSlot);
        let json = serde_json::to_string(&s).unwrap();
    ...
    fn scalar_amplitude_slot() -> SrcSlot {
        SrcSlot {
            shape: SrcShape::Scalar {
                kind: Kind::Amplitude,
                ...
                default: SrcValueSpec::Literal(WireValue::F32(1.0)),
            },
            label: None,
            description: None,
            bind: None,
    ...
    fn count_slot(n: i32) -> SrcSlot {
        SrcSlot {
            shape: SrcShape::Scalar {
                kind: Kind::Count,
                constraint: Kind::Count.default_constraint(),
                default: SrcValueSpec::Literal(WireValue::I32(n)),
            },
            label: None,
            description: None,
            bind: None,
