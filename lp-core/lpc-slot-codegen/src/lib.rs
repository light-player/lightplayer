...
fn render_mockup_slot_codec() -> String {
    ...
    out.push_str(&render_mockup_source_roots(&mockup_source_codec_module()));
...
fn render_mockup_source_roots(module: &SlotCodecModule) -> String {
    let mut out = String::new();
    for root in &module.roots {
        render_slot_codec_root(&mut out, root);
    }
    out
}

fn render_slot_codec_root(out: &mut String, root: &SlotCodecRoot) {
    render_slot_codec_root_read_wrappers(out, root);
    render_slot_codec_root_reader(out, root);
    render_slot_codec_root_writer(out, root);
}

fn render_slot_codec_root_read_wrappers(out: &mut String, root: &SlotCodecRoot) {
    out.push_str("pub fn read_");
    out.push_str(root.fn_stem);
    out.push_str("_json(json: &str) -> Result<");
    out.push_str(root.rust_type);
...
fn render_slot_codec_root_reader(out: &mut String, root: &SlotCodecRoot) {
    out.push_str("pub fn read_");
    out.push_str(root.fn_stem);
    out.push_str("<S>(reader: &mut SlotReader<'_, S>) -> Result<");
    out.push_str(root.rust_type);
    out.push_str(", SyntaxError>\nwhere\n    S: SyntaxEventSource,\n{\n");
    render_slot_codec_fields_const(out, root);
    if let Some(default_expr) = root.default_expr {
        out.push_str("    let defaults = ");
        out.push_str(default_expr);
        out.push_str(";\n");
    }
    for field in root.fields.iter().filter(|field| !field.skip_read) {
    ...
    out.push_str("    }\n");
    out.push_str("    Ok(");
    out.push_str(root.constructor.expression);
    out.push_str(")\n");
    out.push_str("}\n\n");
}

fn render_slot_codec_root_writer(out: &mut String, root: &SlotCodecRoot) {
    let arg_name = root.fn_stem.strip_suffix("_def").unwrap_or(root.fn_stem);
    ...
    out.push_str(root.kind_expr);
    out.push_str(").unwrap();\n");
    for field in root.fields.iter().filter_map(|field| field.write_expr) {
...
fn render_slot_codec_fields_const(out: &mut String, root: &SlotCodecRoot) {
...
"#,
    ...
    #[test]
    fn mockup_source_codec_module_contains_expected_roots() {
        let module = mockup_source_codec_module();
        let root_types = module
            .roots
            .iter()
            .map(|root| root.rust_type)
            .collect::<Vec<_>>();
    ...
    fn mockup_source_codec_module_contains_representative_fields() {
        ...
        assert_root_has_fields(&module, "ProjectDef", &["name", "nodes"]);
        assert_root_has_fields(&module, "OutputDef", &["pin", "options"]);
        assert_root_has_fields(&module, "FixtureDef", &["mapping"]);
        assert_root_has_fields(&module, "ShaderDef", &["param_defs"]);
    }

    fn assert_root_has_fields(module: &SlotCodecModule, root_type: &str, expected: &[&str]) {
        let root = module
            .roots
            .iter()
            .find(|root| root.rust_type == root_type)
            .unwrap_or_else(|| panic!("missing root {root_type}"));
        for expected_field in expected {
            assert!(
                root.fields
                    .iter()
                    .any(|field| field.wire_name == *expected_field),
                "missing field {root_type}.{expected_field}"
            );
        }
    }
}
