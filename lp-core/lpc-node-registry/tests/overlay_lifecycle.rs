...
#[test]
fn apply_unsupported_slot_op_errors() {
    let mut registry = NodeDefRegistry::new();
    let err = registry
        .apply_change(&ArtifactChange {
            target: ArtifactTarget::Path(LpPathBuf::from("/clock.toml")),
            ops: vec![ArtifactOp::OptionSet {
                path: lpc_model::SlotPath::root(),
                present: true,
            }],
        })
        .unwrap_err();
    assert!(matches!(err, ChangeError::UnsupportedOp { .. }));
    assert!(!registry.overlay_active());
}
