...
use crate::{
    ...
    SlotMapKey, SlotPath, SlotPathSegment, SlotShape, SlotShapeRegistry,
};
use alloc::format;
...
fn lookup_in_shape_mut<'a>(
    ...
    match (shape, data, head) {
        (
            SlotShape::Record { fields, .. },
            SlotDataAccessMut::Record(record),
            SlotPathSegment::Field(name),
        ) => {
            let (index, field) = fields
                .iter()
        ...
        (SlotShape::Map { value, .. }, SlotDataAccessMut::Map(map), SlotPathSegment::Key(key)) => {
            lookup_map_key_mut(map, key, value, registry, tail)
        }
        (
            SlotShape::Option { some, .. },
            SlotDataAccessMut::Option(option),
            SlotPathSegment::Field(name),
        ) if name.as_str() == "some" => {
            let data = option
                .data_mut()
                .ok_or_else(|| SlotLookupError::new("option slot is none"))?;
            lookup_in_shape_mut(data, some, registry, tail)
        }
        (
            SlotShape::Enum { variants, .. },
            SlotDataAccessMut::Enum(en),
            SlotPathSegment::Field(name),
        ) => {
            if en.variant() != name.as_str() {
                return Err(SlotLookupError::new(format!(
