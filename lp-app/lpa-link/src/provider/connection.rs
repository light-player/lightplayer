...
impl LinkConnection {
    ...
    /// A fake connection that carries a REAL host protocol channel: the
    /// scripted `FakeEsp32Device` behind the real serial framing (feature
    /// `fake-device`). Same shape as [`Self::host_serial_esp32`], but the
    /// `Fake` kind keeps callers honest about what they connected to.
