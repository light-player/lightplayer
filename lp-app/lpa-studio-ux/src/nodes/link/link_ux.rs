...
use crate::{
    ...
    ProgressState, ProviderChoice, UiAction, UiActivity, UiBody, UiMetric, UiPaneView, UiProgress,
    UiStatus, UxError, UxIssue, UxLogEntry, UxLogLevel, UxNode, UxNodeId, UxUpdate, UxUpdateSink,
};
...
fn link_body(state: &LinkState) -> UiBody {
    match state {
        ...
        LinkState::DiscoveringEndpoints {
            provider_id,
            progress,
        } => UiBody::Progress(progress.clone().with_detail(format!(
            "Discovering endpoints from {}.",
            provider_id.label()
        ))),
        LinkState::SelectingEndpoint { endpoints, .. } => endpoints
            .first()
            .map(|endpoint| UiBody::text(endpoint.summary.clone()))
            .unwrap_or_else(|| UiBody::text("No endpoints are available for this provider.")),
        LinkState::Connecting { progress, .. } => UiBody::Progress(progress.clone()),
        LinkState::Managing { progress, .. } => UiBody::Progress(progress.clone()),
        LinkState::Connected { device } => UiBody::Metrics(vec![
        ...
        LinkState::Failed { issue } => UiBody::Issue(issue.clone()),
    }
}
