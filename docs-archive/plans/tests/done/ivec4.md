...
Create a complete test suite for integer vector type `ivec4` in `lightplayer/crates/lps-filetests/filetests/ivec4/` following the organizational pattern used in `vec4/` and `bool/`. These tests will comprehensively cover the GLSL signed integer vector specification for `ivec4` and are expected to fail initially, serving as a specification for implementing the rest of the compiler.
