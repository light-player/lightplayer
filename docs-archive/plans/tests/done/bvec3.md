...
Create a complete test suite for boolean vector type `bvec3` in `lightplayer/crates/lps-filetests/filetests/bvec3/` following the organizational pattern used in `vec4/` and `bool/`. These tests will comprehensively cover the GLSL boolean vector specification for `bvec3` and are expected to fail initially, serving as a specification for implementing the rest of the compiler.
...
- Pattern: `lightplayer/crates/lps-filetests/filetests/vec4/constructors/shortening.glsl` (for from-shortening.glsl)
