...
Add Cargo **feature** `riscv32-emu` (name locked for this plan) and **optional dependencies** for
object emission and RISC-V:
...
`cargo test -p lpvm-cranelift` does not require builtins artifacts or emulator.
...
- Default features for `lpvm-cranelift` should remain usable for host JIT only.
...
- No behavioral tests yet; confirm `cargo check -p lpvm-cranelift` and
  `cargo check -p lpvm-cranelift --features riscv32-emu` both succeed (once
  follow-on phases fill in modules, or use empty cfg modules).

## Validate

```bash
cd /Users/yona/dev/photomancer/lp2025/lps && cargo check -p lpvm-cranelift
cd /Users/yona/dev/photomancer/lp2025/lps && cargo check -p lpvm-cranelift --features riscv32-emu
```
