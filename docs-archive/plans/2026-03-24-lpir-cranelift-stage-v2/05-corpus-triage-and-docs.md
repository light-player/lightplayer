...
- **README:** document **`DEFAULT_TARGETS` = `jit.q32` only**; document **CI**
  ...
  `lps-filetests-app test --target wasm.q32` for each).
...
- Full **`cargo test -p lps-filetests --test filetests`** locally with
  **`--target`** or env to sweep all targets when ready.

## Validate

```bash
cd /Users/yona/dev/photomancer/lp2025/lps && cargo test -p lps-filetests --test filetests
