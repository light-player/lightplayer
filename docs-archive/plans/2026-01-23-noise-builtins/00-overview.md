...
- Hash function (`lpfn_hash`) with 1D, 2D, and 3D variants
- Simplex noise functions (`lpfn_snoise1`, `lpfn_snoise2`, `lpfn_snoise3`)
- Integration with the existing builtin system
- Semantic checking and codegen for user-facing `lp_*` function names

## Phases

1. **Implement hash function** - Create `lpfn_hash.rs` with Q32 fixed-point hash functions using noiz algorithm
2. **Implement Simplex noise functions** - Create `lpfn_snoise1.rs`, `lpfn_snoise2.rs`, `lpfn_snoise3.rs` with Q32 implementations
