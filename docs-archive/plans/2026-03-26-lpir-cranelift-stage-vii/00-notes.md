...
- `#[ignore]` on `lpfn_builtins_memory.rs` test: WASM ABI mismatch for vec3
...
### Q5 — Ignored test `lpfn_builtins_memory.rs`

**Context:** `#[ignore]` with note about WASM ABI mismatch for
`lpfn_saturate_vec3` (multi-return vs result-pointer). Roadmap says re-check
in Stage VII.
...
- Q5: Un-ignore `lpfn_builtins_memory` test; re-ignore with updated comment
