...
1. **Compiler frontend**: Resolves function call to `LpfnFn` → gets `float_impl` `BuiltinId` → generates call to `builtin_id.name()` (e.g., `__lpfn_saturate_vec3_f32`)
2. **Q32 Transform**: Sees `__lpfn_saturate_vec3_f32` in CLIF → looks up `BuiltinId` from name → finds corresponding `LpfnFn` → replaces with `q32_impl` `BuiltinId` → uses `builtin_id.name()` (e.g., `__lpfn_saturate_vec3_q32`)
