...
- Implement both `saturate` and `hue2rgb` following lygia's structure:
    - `saturate` in `lpfn/math/` (following lygia's structure)
    - `hue2rgb` in `lpfn/color/space/` (color-specific)
...
**Context:** Looking at existing lpfn functions (like `snoise`), they have both `_f32` and `_q32`
...
stubs later following the same pattern as other lpfn functions if needed.
...
`lp-shader/lps-builtins/src/builtins/lpfn`. Lygia has:
...
- `lp-shader/lps-builtins/src/builtins/lpfn/color/space/` for HSV functions and `hue2rgb`
- `lp-shader/lps-builtins/src/builtins/lpfn/math/` for `saturate`
...
- Create `lpfn/math/` for math utilities
- Put it in `lpfn/color/space/` since it's primarily used by color functions
- Put it in `lpfn/color/` as a general color utility

**Suggested Answer:** Create `lpfn/math/` subdirectory for math utilities like `saturate`, following
