...
In filetests detail mode, error formatting is inconsistent across different error paths. The error
details and rerun commands should always appear at the end of the output, so when you tail the
command output, you see the most important information (error details and how to rerun).
...
Each path has different ordering of sections, making it hard to find the error details and rerun
commands.
...
The goal is that when you tail the command output, you'll see the error details and rerun commands
at the end.
...
2. ✅ **How should we handle the DEBUG mode check? Should it be passed as a parameter or checked via
   environment variable?**
   - **Answer:** Replace `show_full_output` boolean with an enum:
     `OutputMode { Summary, Detail, Debug }`
...
3. ✅ **Should the rerun commands include both `DEBUG=1` and non-DEBUG versions?**
   - **Answer:** Yes, show both commands with labels:
     ...
     Rerun just this test:
       scripts/filetests.sh matrix/mat4/op-add.glsl:42

     Rerun with debugging:
       DEBUG=1 scripts/filetests.sh matrix/mat4/op-add.glsl:42
     ```

4. ✅ **Should we extract the debug info formatting (emulator state, v-code, CLIF) into separate
   functions for better organization?**
   - **Answer:** Keep one function (`format_debug_info`) to ensure consistent section headers.
     Reorder sections to match desired order.

5. ✅ **How should we handle cases where the executable is not available (e.g., compilation errors)?
   Should we skip debug sections or show what we can?**
   - **Answer:** Pass `None` for the executable parameter. Consider higher-level wrapper
     functions (e.g., `format_compilation_error_detail`, `format_execution_error_detail`) that
     handle specific error cases and delegate to the main formatter, to avoid having to pass lots
     of `None` values.

6. ✅ **Should the GLSL section always show the bootstrap code, or should it show the original file
   context when available?**
   - **Answer:** Always show what was actually compiled (the isolated test code). This matches line
     numbers and only includes functions used in the test for clarity.
   - **Note:** Rename "bootstrap" terminology → "compiled glsl". The code no longer has a main()
     wrapper, so "bootstrap" is misleading.
...
- Update logic that checks `show_full_output`:
  ...
  - `if show_full_output` → `if output_mode != OutputMode::Summary`
- Update call sites:
  - `lib.rs`: `let show_full_output = test_specs.len() == 1;` → determine `OutputMode` based on
    test count and DEBUG env var
...
- `lightplayer/crates/lps-filetests/src/test_run/mod.rs`
- `lightplayer/crates/lps-filetests/src/lib.rs`
- `lightplayer/crates/lps-filetests/src/concurrent.rs`
...
- Update all references:
  - `bootstrap::generate_bootstrap()` → `compiled_glsl::generate_compiled_glsl()` (or keep
    function name)
...
- `lightplayer/crates/lps-filetests/src/test_run/bootstrap.rs` (possibly rename file)
- `lightplayer/crates/lps-filetests/src/test_run/mod.rs`
- `lightplayer/crates/lps-filetests/README.md` (if applicable)
...
- Create new function `format_detail_mode_error` with signature:
  ...
  fn format_detail_mode_error(
      ...
      executable: Option<&dyn lps_compiler::GlslExecutable>,  // For debug sections
...
- Wrappers should handle `None` values gracefully (e.g., pass `None` for executable when not
  available)
