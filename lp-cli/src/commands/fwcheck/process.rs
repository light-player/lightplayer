...
pub fn flash_esp32_no_reset(root: &Path, port: &str, verbose: bool) -> Result<()> {
    flash_esp32_with_after(root, port, "no-reset", verbose)
}

fn flash_esp32_with_after(root: &Path, port: &str, after: &str, verbose: bool) -> Result<()> {
